...
def get_wordnet_subtree(
    start_synset_name="mammal.n.01", depth_limit=3
):
    ...
    while queue:
        ...
        if lemmas:
            ...
            for lem in lemmas[1:]:
                ...
                if (
                    len(temp_name) < len(name)
                    and "-" not in temp_name
                ):
                    name = temp_name
...
def spatial_distance_sq(coord1_spatial, coord2_spatial):
    ...
    return sum(
        (x1 - x2) ** 2
        for x1, x2 in zip(coord1_spatial, coord2_spatial)
    )


def check_causal_link(
    child_coord, parent_coord, epsilon, spatial_dims
):
...
def compute_minkowski_embedding(
    ...
    for i, name in enumerate(name_list):
        coords[name] = [0.0] + spatial_coords[
            i
        ].tolist()  # [t, x, y, ...]
    ...
    for iteration in range(max_iterations):
        ...
        for child_name, parent_name in hierarchy_pairs:
            if child_name not in coords or (
                parent_name and parent_name not in coords
            ):
                continue

            child_coord = coords[child_name]
            parent_coord = (
                coords[parent_name] if parent_name else None
            )

            if not check_causal_link(
                child_coord, parent_coord, epsilon, spatial_dims
            ):
                violations += 1
                if parent_coord:
                    ...
                    required_time_diff = (
                        np.sqrt(spatial_dist_sq + epsilon) + 0.001
                    )
                    coords[child_name][0] = (
                        parent_coord[0] + required_time_diff
                    )
...
class INNPhi(nn.Module):
    ...
    def __init__(
        self, input_dim=4, hidden_dim=128, num_coupling_layers=4
    ):
        super().__init__()
        self.input_dim = input_dim
        ...
        for i in range(num_coupling_layers):
            ...
            self.coupling_layers.append(
                AdditiveCouplingLayer(
                    input_dim, hidden_dim, mask_type
                )
            )
...
def compute_jacobian(phi_model, m_coords_tensor):
    ...
    for i in range(4):
        e_coords = phi_model(
            m_coords_tensor.unsqueeze(0), reverse=False
        ).squeeze()
...
def analyze_local_geometry(m_coord_np, phi_model, eta_E):
    ...
    m_tensor = torch.tensor(
        m_coord_np, dtype=torch.float32, device=DEVICE
    )
...
def train_inn_phi_embedding(
    ...
    optimizer = optim.Adam(
        inn_phi_model.parameters(), lr=learning_rate
    )

    # Initialize m_coords randomly
    num_tokens = len(tokens)
    m_coords_map = {
        token: torch.randn(4, device=DEVICE) * 0.1 for token in tokens
    }
    ...
    for epoch in range(num_epochs):
        ...
        for child_token, parent_token in hierarchy_pairs:
            ...
            child_e = inn_phi_model(
                child_m.unsqueeze(0), reverse=False
            ).squeeze()

            if parent_token:
                parent_m = m_coords_map[parent_token]
                parent_e = inn_phi_model(
                    parent_m.unsqueeze(0), reverse=False
                ).squeeze()
        ...
        if epoch % metric_reg_frequency == 0 and epoch > 0:
            ...
            for token in sample_tokens:
                try:
                    ...
                    e_coord = inn_phi_model(m_coord.unsqueeze(0), reverse=False).squeeze()
        ...
        # Progress reporting
        if epoch % 50 == 0:
            print(f"Epoch {epoch}, Loss: {total_loss.item():.6f}, Causal violations: {causal_violations}")

    # Convert final coordinates to numpy
    final_m_coords = {
        token: coords.detach().cpu().numpy()
        for token, coords in m_coords_map.items()
    }
...
def plot_3d_manifold(
    ...
    if color_values is not None:
        scatter = ax.scatter(x_coords, y_coords, z_coords, 
                           c=color_values, s=100, cmap="viridis", alpha=0.8)
    ...
    for child, parent in hierarchy_pairs:
        if (
            child in m_coords_map
            and parent
            and parent in m_coords_map
        ):
...
def plot_metric_analysis(
    m_coords_map, phi_model, eta_E, hierarchy_pairs
):
    ...
    for token in tokens:
        analysis = analyze_local_geometry(
            m_coords_map[token], phi_model, eta_E
        )
    ...
    for sig in signatures:
        sig_str = str(sig)
        signature_counts[sig_str] = (
            signature_counts.get(sig_str, 0) + 1
        )
...
mammal_concepts = get_wordnet_subtree("mammal.n.01", depth_limit=3)
...
for name in mammal_concepts.keys():
    try:
        concept_embeddings[name] = get_bert_embedding(
...
# %%
# Build hierarchy
hierarchy_pairs, root_name = build_hierarchy_from_concepts(
    mammal_concepts
)
hierarchy_pairs = [
    (child, parent)
    for child, parent in hierarchy_pairs
    if child in concept_embeddings
    and (parent is None or parent in concept_embeddings)
]
print(
    f"Built hierarchy with {len(hierarchy_pairs)} pairs, root: {root_name}"
)
...
print(
    f"Computed Minkowski embedding for {len(minkowski_coords)} concepts"
)
...
final_coords, trained_model = train_inn_phi_embedding(
    ...
    hierarchy_pairs,
    inn_model,
    num_epochs=200,  # Reduced epochs for faster training
...
plot_metric_analysis(
    final_coords, trained_model, eta_E, hierarchy_pairs
)

# %%
print(
    "Refactoring complete! The notebook is now organized into logical cells with:"
)
print("- All imports consolidated at the top")
print("- Duplicated functions removed")
print("- Clean notebook cell structure with # %%")
print("- Minimal comments focused on key insights")
print(
    "- Working demonstration with WordNet mammals and BERT embeddings"
)
