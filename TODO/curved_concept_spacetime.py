...
1.  **The "Magic Map"**
    ...
    * On this map, how ideas are connected (like "dog is a type of mammal") is shown by special lines that are almost like light beams. This is called "causality" – meaning one idea leads to another.
...
with get_mammal_concepts(
    depth_limit=NUM_DIMENSIONS, start_synset_name="mammal.n.0!"
) as carnivore_data_wn:
...
def embed_hierarchy_curved(
    tokens, hierarchy_pairs, metric_func, epsilon1=1e-5, epsilon2=0, max_iterations=1000
):
    """
...
flat_events = embed_hierarchy_curved(
    tokens, hierarchy_pairs, flat_minkowski_metric, max_iterations=200
)

# 3. Run the embedding with the toy curved metric
print("\n--- Embedding with Toy Curved Metric ---")
curved_events = embed_hierarchy_curved(
    tokens, hierarchy_pairs, toy_curved_metric, max_iterations=200
)
...
def plot_embedding(event_map, title="Spacetime Embedding"):
    ...
    scatter = ax.scatter(
        points[:, 1], points[:, 2], points[:, 0], c=points[:, 3], cmap="viridis", s=100
    )
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
...
plot_embedding(
    curved_events, "Toy Curved Spacetime Embedding of Hierarchy (Z as color)"
)

print("\n--- Understanding the Current 'Curved' Embedding and Path to True GR ---")
print(
    "The `toy_curved_metric` and its use in `embed_hierarchy_curved` provide a conceptual step towards curved spacetime."
)
print(
    "However, it's important to understand the simplifications made and what a rigorous General Relativistic (GR) approach entails:"
)
print("\n1. Metric Tensor (g_munu):")
print(
    "   - Current: `toy_curved_metric` is a heuristic, not derived from physical principles. It makes g_00 (time component) and spatial components (g_ii) vary with position."
)
print(
    "   - GR Concept: In GR, g_munu is derived from Einstein's Field Equations (G_munu = 8piG T_munu), which relate spacetime geometry to the distribution of mass-energy. Specific solutions like Schwarzschild (for a static, spherically symmetric mass) or Kerr (for a rotating mass) provide precise, complex forms for g_munu."
)
print(
    "   - Conceptual Landscape: For a 'conceptual spacetime', the 'mass-energy' could be analogous to information density, concept centrality, or some other measure of conceptual 'gravity'."
)

print("\n2. Proper Interval (ds^2 = g_munu dx^mu dx^nu):")
print(
    "   - Current: `proper_interval_sq` calculates ds^2 between two events using dx^mu (coordinate differences) and g_munu evaluated at the *midpoint*. This is an approximation."
)
print(
    "   - GR Concept: The true proper interval (or proper time for timelike paths) between two events is found by integrating the metric along the *geodesic path* connecting them: tau = integral sqrt(-ds^2). For small separations, the midpoint approximation can be reasonable, but for larger separations, it deviates."
)

print("\n3. Geodesics (Paths of 'Free' Motion/Information Flow):")
print(
    "   - Current: The `embed_hierarchy_curved` function adjusts time coordinates heuristically to satisfy ds^2 approx -epsilon1^2 and child.t > parent.t. It does *not* find or enforce geodesic paths."
)
print(
    "   - GR Concept: Particles and light follow geodesics, which are the 'straightest possible lines' in curved spacetime. They are solutions to the geodesic equation:"
)
print(
    "     d^2x^mu / d(tau)^2 + Gamma^mu_alpha_beta * (dx^alpha/d(tau)) * (dx^beta/d(tau)) = 0"
)
print(
    "     where tau is the proper time (or an affine parameter for null geodesics) and Gamma^mu_alpha_beta are the Christoffel symbols (connection coefficients)."
)
print(
    "   - Christoffel Symbols: These are derived from the first derivatives of the metric tensor: Gamma^mu_alpha_beta = (1/2) * g^mu_sigma * (dg_sigma_alpha/dx^beta + dg_sigma_beta/dx^alpha - dg_alpha_beta/dx^sigma)."
)
print(
    "   - Conceptual Landscape: Hierarchical links (parent-child) should ideally correspond to 'almost null' or short timelike geodesics, representing the most direct path for conceptual inheritance or information flow."
)

print("\n4. Coordinate Adjustment Logic:")
print(
    "   - Current: The time adjustment is a simple shift based on the deviation of ds^2 from a target. Spatial coordinates are not adjusted in this iterative step (though initialized by PCA in the LLM example for flat space, and randomly here)."
)
print(
    "   - GR Concept for Embedding: A more rigorous approach would involve an optimization problem. The goal would be to find event coordinates (t, x, y, z) for all concepts suchthat:"
)
print(
    "     a) For each parent-child pair, there exists a geodesic connecting them whose proper time squared is approximately -epsilon1^2 (almost null and timelike)."
)
print(
    "     b) Causal order (child.t > parent.t along the geodesic path) is maintained."
)
print(
    "     This is a complex two-point boundary value problem for geodesics, embedded within an optimization loop."
)

print("\n5. Tools for a GR-based Approach:")
print(
    "   - Symbolic Calculation: Libraries like `sympy` are invaluable for deriving Christoffel symbols from a given metric tensor analytically."
)
print(
    "   - Numerical ODE Solvers: `scipy.integrate.solve_ivp` or `odeint` would be used to numerically solve the geodesic equations once the Christoffel symbols are known."
)
print(
    "   - GR Libraries: `einsteinpy` provides tools for defining metrics, calculating Christoffel symbols, and solving geodesic equations, which can significantly simplify these tasks."
)
print(
    "   - Optimization: `scipy.optimize` or more advanced libraries would be needed for the global optimization problem of placing events."
)

print("\n6. Visualization in Curved Spacetime:")
print(
    "   - Current: 3D scatter plot (x, y, t) with z as color. This is a coordinate representation."
)
print(
    "   - GR Concept: Visualizing true 4D curved spacetime is inherently challenging. Techniques include:"
)
print(
    "     - Embedding diagrams: Slicing the 4D spacetime (e.g., at constant time or equatorial plane) and embedding the resulting 2D or 3D curved manifold into a higher-dimensional flat space (e.g., Flamm's paraboloid for Schwarzschild)."
)
...
print(
    "--- Phase 2 (Simplified): Geodesics in Schwarzschild Spacetime (Manual ODEs) ---"
)
...
def schwarzschild_geodesic_odes(lambda_param, y, M_val):
    ...
    dv_t_dlambda = (
        (-2 * M_val / (r * (r - 2 * M_val))) * vr * vt
    )  # -2 * Gamma^t_rt * vr * vt

    dv_r_dlambda = (
        -(M_val * (r - 2 * M_val) / r**3) * vt**2
        + (M_val / (r * (r - 2 * M_val))) * vr**2
        + (r - 2 * M_val)
        * vth**2  # Note: This term is positive because Gamma^r_thth is negative
        + ((r - 2 * M_val) * np.sin(th) ** 2) * vph**2
    )  # Same for phi term

    dv_th_dlambda = (-2 / r) * vr * vth + (
        np.sin(th) * np.cos(th)
    ) * vph**2  # -2 * Gamma^th_rth * vr * vth - Gamma^th_phph * vph^2

    dv_ph_dlambda = (-2 / r) * vr * vph - (
        2 * np.cos(th) / (np.sin(th) + 1e-9)
    ) * vth * vph  # -2 * Gamma^ph_rph * vr * vph - 2 * Gamma^ph_thph * vth * vph
...
vr_init_timelike = (
    0.0  # Initially no radial velocity *with respect to affine parameter*
)
vth_init_timelike = 0.0
vph_init_timelike = (
    0.00  # Slight angular momentum to see orbit if not falling straight in
)
...
vph_init_null_tangential = (
    0.1 / r_init
)  # give it some tangential velocity for an orbit-like path
...
vt_init_null_orbit = np.sqrt(
    (
        (1 / (1 - 2 * M_val / r_init)) * vr_init_null_outward**2
        + r_init**2 * vph_init_null_tangential**2
    )
    / (1 - 2 * M_val / r_init)
)
...
sol_timelike = solve_ivp(
    schwarzschild_geodesic_odes,
    lambda_span,
    y0_timelike,
    dense_output=True,
    args=(M_val,),
...
t_tl, r_tl, th_tl, ph_tl = (
    traj_timelike_solution[0],
    traj_timelike_solution[1],
    traj_timelike_solution[2],
    traj_timelike_solution[3],
)
...
t_nl, r_nl, th_nl, ph_nl = (
    traj_null_solution[0],
    traj_null_solution[1],
    traj_null_solution[2],
    traj_null_solution[3],
)
...
ax_geo.scatter(
    x_tl[0], y_tl[0], z_tl[0], color="blue", marker="o", s=50, label="Start (Timelike)"
)
ax_geo.scatter(
    x_tl[-1], y_tl[-1], z_tl[-1], color="blue", marker="x", s=50, label="End (Timelike)"
)
...
ax_geo.plot(
    ...
    y_nl,
    z_nl,
    label="Null Geodesic (orbiting outwards)",
    color="orange",
    linestyle="--",
)
ax_geo.scatter(
    x_nl[0], y_nl[0], z_nl[0], color="orange", marker="o", s=50, label="Start (Null)"
)
ax_geo.scatter(
    x_nl[-1], y_nl[-1], z_nl[-1], color="orange", marker="x", s=50, label="End (Null)"
)
...
ax_geo.plot_surface(
    ...
    rstride=5,
    cstride=5,
    label=f"Event Horizon (r_s={schwarzschild_radius})",
)
...
ax_geo.set_zlabel("Z")
ax_geo.set_title(
    f"Example Geodesics in Schwarzschild Spacetime (M={M_val}) - Spatial Projection"
)

# Set limits to better visualize orbits, prevent trajectories from going too far.
max_r_plot = max(
    r_init * 2,
    np.max(r_nl[r_nl < r_init * 5]) if len(r_nl[r_nl < r_init * 5]) > 0 else r_init * 2,
)  # Avoid extreme zoom out if one particle escapes far
...
print(
    "1. Geodesic as the Path: The interval between two events for the 'almost null' condition should be calculated along the *geodesic* connecting them, not a straight line in coordinate space with a midpoint metric."
)
print(
    "2. Two-Point Boundary Value Problem: The core of a GR-based embedding algorithm would be to iteratively find coordinates for events (P, C) and the *initial velocity* at P such that the resulting geodesic from P hits C, and the proper time along this geodesic squared is ~ -epsilon^2."
)
print(
    "3. Optimization: This becomes a very complex optimization problem, likely requiring techniques like the shooting method or variational principles combined with numerical ODE solvers."
)
print(
    "4. Metric Choice/Learning: For a conceptual landscape, the Schwarzschild metric is an analogy. A true 'conceptual metric' might be learned from data or derived from a theory of 'conceptual gravity'."
)
print(
    "5. Computational Cost: Repeatedly solving geodesic BVPs for all pairs in a hierarchy is highly computationally intensive."
)
...
def phi_toy_curvature(m_coords, k_spatial=0.1, k_time_radial=0.05, M_conceptual=0.5):
    ...
    time_radial_factor = 1 - M_conceptual / (
        np.sqrt(m_r_sq) + 1e-3
    )  # Similar to g_tt in Schwarzschild

    e_coords[0] = (
        m_t * time_radial_factor
    )  # * (1 + k_time_radial * m_t) # Optional self-time warping

    return e_coords


def phi_inverse_toy_curvature(
    e_coords, k_spatial=0.1, k_time_radial=0.05, M_conceptual=0.5
):
    """
    ...
    m_r_approx = np.sqrt(
        e_r_sq_approx
    )  # Very rough if spatial_warp_factor is not close to 1
    ...
    if k_spatial == 0:
        ...
        spatial_unwarp_factor_approx = (
            1 + k_spatial * e_r_sq_approx
        )  # This is incorrect for inverse
        ...
        if spatial_unwarp_factor_approx == 0:
            spatial_unwarp_factor_approx = 1  # avoid division by zero
    ...
    if time_radial_factor_approx == 0:
        time_radial_factor_approx = 1e-9  # avoid division by zero
...
class ConceptualEvent:
    ...
    def e_t(self, value):
        ...
        self.m_coords = self.phi_inverse(
            self.e_coords
        )  # Update m_coords if e_coords change
...
def embed_hierarchy_diffeomorphism(
    tokens,
    hierarchy_pairs,
    phi_func,
    phi_inv_func,
    ...
    max_iterations=1000,
    k_spatial_phi=0.1,
    k_time_radial_phi=0.05,
    M_conceptual_phi=0.5,
):  # phi params
    """
    ...
    event_map = {
        token_id: ConceptualEvent(
            token_id,
            lambda m: phi_func(m, k_spatial_phi, k_time_radial_phi, M_conceptual_phi),
            lambda e: phi_inv_func(
                e, k_spatial_phi, k_time_radial_phi, M_conceptual_phi
            ),
        )
    ...
    for iteration in range(max_iterations):
        ...
        for child_id, parent_id in hierarchy_pairs:
            ...
            if child_event.e_t <= parent_event.e_t:
                ...
                child_event.e_t += delta * (
                    1 - epsilon2
                )  # This updates e_coords and m_coords via setter
                parent_event.e_t -= delta * epsilon2
                continue

            # "Almost null" or timelike check in E
            if (
                ds_sq_E > 0 or ds_sq_E < target_ds_sq_E
            ):  # Spacelike or "too" timelike in E
                violations_found = True
                ...
                if (
                    spatial_dist_sq_E - target_ds_sq_E < 0
                ):  # Should not happen if epsilon1 is small
                    ...
                    target_new_delta_e_t = (
                        np.sqrt(spatial_dist_sq_E + epsilon1**2 * 0.1) + 1e-4
                    )  # Ensure > D_XY
                else:
        ...
        if iteration == max_iterations - 1:
            print(
                f"Max iterations ({max_iterations}) reached for E-space. Not fully converged."
            )
...
diffeo_m_coords_map = embed_hierarchy_diffeomorphism(
    ...
    hierarchy_pairs,
    phi_toy_curvature,
    phi_inverse_toy_curvature,
    epsilon1=1e-4,
    max_iterations=500,  # Adjusted epsilon, more iterations
    **phi_params,
)
...
def plot_m_embedding(
    m_coords_map_plot, title="Conceptual Manifold Embedding (m_coords)"
):
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection="3d")
    ...
    scatter = ax.scatter(
        points[:, 1], points[:, 2], points[:, 0], c=points[:, 3], cmap="plasma", s=100
    )
    ...
    for child_id, parent_id in hierarchy_pairs:
        if (
            parent_id is None
            or child_id not in m_coords_map_plot
            or parent_id not in m_coords_map_plot
        ):
            continue
...
plot_m_embedding(
    diffeo_m_coords_map,
    f"Conceptual Landscape (m_coords) via Diffeomorphism (phi_toy_curvature with k_s={phi_params['k_spatial_phi']}, M_c={phi_params['M_conceptual_phi']})",
)


print("\n--- Discussion on Diffeomorphism Approach ---")
print(
    "1. Effective Curvature: The `phi_toy_curvature` function introduces non-linear transformations. When we embed in the flat 'E-space' (Minkowski) and then map back to 'M-space' using `phi_inverse`, the geometry in M-space is effectively curved. Straight lines (almost null geodesics) in E-space become curved paths in M-space."
)
print(
    "2. Choice of Phi: The nature of the curvature is entirely determined by `phi`. A more principled `phi` could be derived from assumptions about how conceptual density or importance warps conceptual 'distance' and 'time'."
)
print(
    "3. Inverse Phi: A robust `phi_inverse` is crucial. For complex `phi`, this might require numerical methods (e.g., `scipy.optimize.fsolve`) to find `m_coords` given `e_coords` by solving `phi(m_coords) - e_coords = 0`."
)
print(
    "4. Computational Advantage: All iterative adjustments and interval calculations happen in the flat Minkowski E-space, avoiding direct, complex GR calculations (Christoffel symbols, geodesic ODEs) within the optimization loop."
)
print(
    "5. Interpretation: The resulting `m_coords` represent concepts in a landscape where distances and causal relations are non-trivial. For instance, two concepts might be far apart in `m_spatial` coordinates but 'causally close' because `phi` maps them to be so in E-space."
)
print("6. Next Steps (Advanced):")
print(
    "   - Design more sophisticated `phi` functions based on conceptual hypotheses (e.g., making 'time' flow slower near 'dense' concepts, or warping space around influential ideas)."
)
print(
    "   - Explore learning `phi` itself from data, as suggested by the Riemannian Flow Matching paper, to discover the intrinsic geometry of the conceptual hierarchy."
)
print(
    "   - For analysis on M, one could compute the pullback metric: g_M = (J_phi)^T g_E J_phi, where J_phi is the Jacobian of phi. This g_M would be the explicit metric tensor of your curved conceptual manifold."
)
...
def phi_inverse_toy_curvature_numerical(
    e_coords_target, k_spatial=0.1, k_time_radial=0.05, M_conceptual=0.5
):
    """
    ...
    def objective_function(m_coords_guess):
        ...
        return (
            phi_toy_curvature(m_coords_guess, k_spatial, k_time_radial, M_conceptual)
            - e_coords_target
        )
    ...
    if k_spatial == 0:  # If no spatial warping, m_spatial = e_spatial
        ...
        def spatial_objective(m_spatial_guess):
            ...
            e_spatial_phi = phi_toy_curvature(
                m_coords_for_phi, k_spatial, k_time_radial, M_conceptual
            )[1:]
            return e_spatial_phi - e_coords_target[1:]

        initial_spatial_guess = e_coords_target[1:]
        m_spatial_found, info_spatial, ier_spatial, msg_spatial = fsolve(
            spatial_objective, initial_spatial_guess, full_output=True
        )
        if ier_spatial != 1:
            print(
                f"Warning: fsolve for spatial part failed for e_coords {e_coords_target}: {msg_spatial}"
            )
            # Fallback to a simpler approximation if fsolve fails
            e_r_sq = np.sum(e_coords_target[1:] ** 2)
            approx_factor_inv = (
                1 + k_spatial * e_r_sq
            )  # This is actually spatial_warp_factor for m_r=e_r
...
class ConceptualEvent:  # Redefine to ensure it uses the new inverse
    def __init__(
        ...
        phi_func,
        phi_inv_func,
        initial_m_coords=None,
        phi_params_dict=None,
    ):  # Added phi_params_dict
        self.token_id = token_id
        ...
        self.phi_inverse = lambda e: phi_inv_func(
            e, **self._phi_params
        )  # Pass params to phi_inverse
...
def jacobian_phi_toy_curvature(
    m_coords, k_spatial=0.1, k_time_radial=0.05, M_conceptual=0.5
):
    """
...
def embed_hierarchy_diffeomorphism(
    ...
    hierarchy_pairs,
    phi_func,
    phi_inv_func,  # Now expects the numerical inverse
    epsilon1=1e-5,
    epsilon2=0,
    max_iterations=2000,
    ...
    event_map = {
        token_id: ConceptualEvent(
            token_id, phi_func, phi_inv_func, phi_params_dict=phi_params_dict_for_event
        )
        for token_id in tokens
    }

    print(
        f"Starting embedding with diffeomorphism: {phi_func.__name__} (using numerical inverse)"
    )
    ...
    for iteration in range(max_iterations):
        ...
        for child_id, parent_id in hierarchy_pairs:
            if parent_id is None:
                continue  # Skip root links for interval check
            child_event = event_map[child_id]
            parent_event = event_map[parent_id]
            ...
            if child_event.e_t <= parent_event.e_t:
                violations_found = True
                delta = (
                    parent_event.e_t - child_event.e_t
                ) + 0.01  # smaller push for numerical inverse
            ...
            if ds_sq_E > 0 or ds_sq_E < target_ds_sq_E:
                ...
                if spatial_dist_sq_E - target_ds_sq_E < 0:
                    target_new_delta_e_t = (
                        np.sqrt(spatial_dist_sq_E + epsilon1**2 * 0.01) + 1e-5
                    )
                else:
                    target_new_delta_e_t = np.sqrt(spatial_dist_sq_E - target_ds_sq_E)
                ...
                current_child_et = (
                    child_event.e_t
                )  # Re-fetch in case other adjustments happened
                current_parent_et = parent_event.e_t
        ...
        if not violations_found:
            ...
            break
        if iteration % 50 == 0:
            print(f"Iteration {iteration}...")  # Progress
        if iteration == max_iterations - 1:
            print(
                f"Max iterations ({max_iterations}) reached for E-space. Not fully converged."
            )
...
hierarchy_pairs_pb = [
    ...
    ("milky_way", "galaxy"),
    ("solar_system", "star_system"),
    ("solar_system", "milky_way"),
    ("sun", "solar_system"),
    ("earth", "solar_system"),
]

phi_params_pb = {"k_spatial": 0.05, "k_time_radial": 0.0, "M_conceptual": 0.1}
...
diffeo_m_coords_map_pb = embed_hierarchy_diffeomorphism(
    ...
    hierarchy_pairs_pb,
    phi_toy_curvature,
    phi_inverse_toy_curvature_numerical,  # Pass the numerical inverse
    epsilon1=1e-4,
    max_iterations=2000,  # Fewer iterations for demo speed
    phi_params_dict_for_event=phi_params_pb,
)

plot_m_embedding(
    diffeo_m_coords_map_pb,
    f"Manifold (m_coords) via Numerical Inverse Phi (k_s={phi_params_pb['k_spatial']}, M_c={phi_params_pb['M_conceptual']})",
)
...
print(
    "g_M(m_coords) tells you how intervals (ds^2) are measured *on the conceptual manifold M* at point m_coords."
)
print(
    "- g_M[0,0]: The squared rate at which conceptual proper time (on M) passes relative to coordinate time m_t, at that point."
)
print(
    "  If g_M[0,0] is more negative than -1, conceptual proper time flows 'slower' than m_t."
)
print("- g_M[i,i] (i=1,2,3): Relate to spatial distances along coordinate axes on M.")
print(
    "  If g_M[i,i] > 1, coordinate distance dm_i corresponds to a larger proper distance on M (space is 'stretched')."
)
print(
    "  If g_M[i,i] < 1, coordinate distance dm_i corresponds to a smaller proper distance on M (space is 'compressed')."
)
print(
    "- Off-diagonal terms g_M[i,j] (i!=j): Indicate how spacetime axes are tilted or sheared relative to each other on M."
)
print(
    "- det(g_M): The determinant of the metric is related to the volume element. In flat Minkowski, det(eta)=-1."
)
...
print(
    "Further analysis could involve calculating curvature scalars (Ricci scalar) from g_M, which is more involved."
)
...
class PhiNetwork(nn.Module):
    def __init__(self, input_dim=4, output_dim=4, hidden_dims=[64, 128, 64]):
        ...
        for h_dim in hidden_dims:
            ...
            layers.append(
                nn.Tanh()
            )  # Using Tanh for bounded activations, can experiment
            prev_dim = h_dim
...
class ConceptualEventLearnedPhi:
    ...
    def get_e_coords(self):
        ...
        return self.phi_nn(self.initial_m_coords.unsqueeze(0)).squeeze(
            0
        )  # Add batch dim, then remove
...
def train_phi_embedding(
    ...
    initial_m_coords_map = {
        token_id: np.array(
            [0.0] + np.random.uniform(-1, 1, 3).tolist(), dtype=np.float32
        )
        for token_id in tokens
    }
    ...
    event_objects = {
        token_id: ConceptualEventLearnedPhi(
            token_id, initial_m_coords_map[token_id], phi_nn
        )
        for token_id in tokens
    }
    ...
    for child_id, parent_id in hierarchy_pairs:
        ...
        ):  # We only care about actual parent-child links for loss
            if child_id in event_objects and parent_id in event_objects:
                training_pairs.append(
                    (event_objects[child_id], event_objects[parent_id])
                )
    ...
    for epoch in range(num_epochs):
        ...
        for child_event, parent_event in training_pairs:
            ...
            delta_e_spatial_sq = torch.sum(
                (e_coords_child[1:] - e_coords_parent[1:]) ** 2
            )
            ds_sq_E = -(delta_e_t**2) + delta_e_spatial_sq
        ...
        if epoch % (num_epochs // 10 or 1) == 0:
            print(
                f"Epoch {epoch + 1}/{num_epochs}, Avg Loss: {total_loss / len(training_pairs):.6f}"
            )
...
initial_m_coords_map_trained, final_e_coords_map_trained, trained_phi_model = (
    train_phi_embedding(
        tokens_lphi,
        hierarchy_pairs_lphi,
        phi_model,
...
def plot_coords_map(coords_map, title="Coordinates Plot", z_label_suffix=""):
    ...
    for child_id, parent_id in hierarchy_pairs_lphi:
        if (
            parent_id is None
            or child_id not in coords_map
            or parent_id not in coords_map
        ):
            continue
...
plot_coords_map(
    initial_m_coords_map_trained,
    "Initial Conceptual Manifold Coords (m_coords - Inputs to Phi)",
)
for i in initial_m_coords_map_trained:
    print(i, initial_m_coords_map_trained[i])
    print(
        f"determinant of g_M at {i}: {np.linalg.det(pullback_metric_gM(initial_m_coords_map_trained[i], **phi_params_pb)):.3e}"
    )
plot_coords_map(
    final_e_coords_map_trained,
    "Final Embedding Space Coords (e_coords - Outputs of Phi)",
)
...
print(
    "1. Learned Phi: The `trained_phi_model` now represents our diffeomorphism phi_theta."
)
print(
    "2. M-coords vs E-coords: We plotted the initial `m_coords` (which were random inputs) and the final `e_coords` (where the Minkowski embedding was enforced). The 'true' conceptual manifold positions that correspond to the learned `e_coords` would be `phi_inverse(e_coords)`."
)
print(
    "3. Numerical Inverse of NN Phi: To get these 'true' M-coords, we'd need to numerically invert `trained_phi_model`. This can be done using `scipy.optimize.fsolve` for each `e_coords_point` by solving `trained_phi_model(m_guess) - e_coords_point = 0`."
)
print("4. Pullback Metric g_M:")
print(
    "   - Once we have `phi_inverse` (even numerically), we can get the Jacobian of `phi_nn` (J_phi) using `torch.autograd.functional.jacobian`."
)
...
print(
    "5. Visualization of M: Plotting the `phi_inverse(final_e_coords_map)` would give the positions on M. Visualizing the curvature could involve plotting Ricci scalar derived from g_M, or showing how test geodesics (calculated using g_M) behave on this learned manifold."
)
print(
    "6. Invertible Neural Networks: For a more robust approach to learning phi and its inverse simultaneously, architectures like Normalizing Flows or other INNs should be explored. This avoids the need for numerical inversion during training or inference."
)
...
if "star" in initial_m_coords_map_trained:
    ...
    m_coord_star_tensor = torch.tensor(
        m_coord_star_np, dtype=torch.float32
    ).requires_grad_(False)  # No grad needed for input
    ...
    J_phi_at_star = torch.autograd.functional.jacobian(
        phi_wrapper_for_jacobian, m_coord_star_tensor
    )
    ...
    print(
        "To get g_M at the 'true' learned position on the manifold, phi_model would need to be inverted first."
    )
...
def phi_inverse_numerical_nn(
    ...
    def objective_function(m_guess_np):
        ...
        e_target_tensor = torch.tensor(
            e_coord_target_np, dtype=torch.float32
        ).unsqueeze(0)
        return (
            (e_output_tensor - e_target_tensor).squeeze(0).numpy()
        )  # fsolve expects numpy array
    ...
    try:
        ...
        if ier == 1:
            ...
            print(
                f"Warning: fsolve did not converge for e_coord {e_coord_target_np}. Message: {msg}. Using initial guess."
            )
            return initial_m_guess_np
    except Exception as e:
        print(
            f"Error during fsolve for e_coord {e_coord_target_np}: {e}. Using initial guess."
        )
        return initial_m_guess_np
...
for token_id, e_final_coord_np in final_e_coords_map_trained.items():
    ...
    initial_m_guess_np = initial_m_coords_map_trained.get(
        token_id, e_final_coord_np.copy()
    )  # Fallback to e_coord if no initial m

    m_star_coord_np = phi_inverse_numerical_nn(
        e_final_coord_np, trained_phi_model, initial_m_guess_np
    )
...
if final_m_coords_on_manifold_map:
    ...
    for token_id in points_to_analyze:
        if token_id in final_m_coords_on_manifold_map:
            ...
            m_final_coord_torch = torch.tensor(
                m_final_coord_np, dtype=torch.float32, requires_grad=False
            )
            ...
            try:
                J_phi_at_m_star = F.jacobian(
                    phi_for_jacobian_final, m_final_coord_torch
                )
...
if final_m_coords_on_manifold_map:
    plot_m_embedding(
        final_m_coords_on_manifold_map, "Learned Conceptual Landscape (Final m_coords)"
    )
else:
...
print(
    "Compare the g_M values and determinants at these 'final' m_coords to those at the initial random m_coords."
)
print(
    "Ideally, the determinants at the final m_coords should be closer to -1 (or a consistent non-zero value),"
)
print(
    "indicating that phi has learned a less degenerate mapping in the regions relevant to your hierarchy."
)
print(
    "The components of g_M now describe the local geometry (time flow rate, spatial stretching/compression, axis tilting)"
)
print(
    "of your *learned* conceptual manifold at the positions where your concepts are actually embedded."
)
...
class AdditiveCouplingLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, mask_type="even"):
        ...
        self.translation_net = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(
                hidden_dim, self.input_dim_half
            ),  # Outputs translation of same dim as transformed part
        )
...
class INNPhi(nn.Module):
    def __init__(self, input_dim=4, hidden_dim=128, num_coupling_layers=4):
        ...
        assert input_dim % 2 == 0, (
            "INNPhi requires even input_dim for current AdditiveCouplingLayer design"
        )
...
class ConceptualEventINNPhi:
    def __init__(self, token_id, initial_m_coords_np, inn_phi_model):
        self.token_id = token_id
        self.initial_m_coords_tensor = torch.tensor(
            initial_m_coords_np, dtype=torch.float32
        )
    ...
    def get_e_coords_tensor(self):
        ...
        return self.inn_phi(
            self.initial_m_coords_tensor.unsqueeze(0), reverse=False
        ).squeeze(0)
...
def train_inn_phi_embedding(
    ...
    initial_m_coords_map = {
        token_id: np.array(
            [0.0] + np.random.uniform(-1, 1, inn_phi_model.input_dim - 1).tolist(),
            dtype=np.float32,
        )
        for token_id in tokens
    }
    event_objects = {
        token_id: ConceptualEventINNPhi(
            token_id, initial_m_coords_map[token_id], inn_phi_model
        )
        for token_id in tokens
    }
    ...
    for child_id, parent_id in hierarchy_pairs:
        if (
            parent_id is not None
            and child_id in event_objects
            and parent_id in event_objects
        ):
    ...
    if not training_pairs:
        ...
        return (
            initial_m_coords_map,
            {name: obj.e_coords for name, obj in event_objects.items()},
            inn_phi_model,
        )
    ...
    for epoch in range(num_epochs):
        ...
        for child_event, parent_event in training_pairs:
            ...
            if log_det_jacobian_weight > 0:
                ...
                log_det_J_child = inn_phi_model.log_det_jacobian(
                    child_event.initial_m_coords_tensor.unsqueeze(0)
                )
                log_det_J_parent = inn_phi_model.log_det_jacobian(
                    parent_event.initial_m_coords_tensor.unsqueeze(0)
                )
            ...
            loss = (
                loss_causal_order
                + loss_interval
                + log_det_jacobian_weight * loss_log_det_J
            )
...
inn_phi_network = INNPhi(
    input_dim=4, hidden_dim=64, num_coupling_layers=6
...
plot_coords_map(
    initial_m_coords_inn,
    "Initial Conceptual Manifold Coords (m_coords - Inputs to INN Phi)",
)
    ...
    for token_id in points_to_analyze_inn:
        if token_id in initial_m_coords_inn:
            ...
            def phi_for_jacobian_inn(m_input_tensor):
                return trained_inn_phi(
                    m_input_tensor.unsqueeze(0), reverse=False
                ).squeeze(0)

            try:
                J_phi_at_m = torch.autograd.functional.jacobian(
                    phi_for_jacobian_inn, m_coord_torch
                )
                ...
                print(
                    f"Log Det Jacobian from INN (should be ~0 for additive): {trained_inn_phi.log_det_jacobian(m_coord_torch.unsqueeze(0)).item():.3e}"
                )
            except Exception as e_jac_inn:
                print(
                    f"Error calculating Jacobian or g_M for '{token_id}' with INN: {e_jac_inn}"
                )
else:
...
print(
    "With an INN (especially purely additive coupling), the log_det_jacobian of phi is often 0 (or constant if scaling is identity)."
)
print("This means det(J_phi) = 1 (or constant).")
print(
    "Therefore, det(g_M) = det(J_phi^T * eta_E * J_phi) = det(J_phi^T) * det(eta_E) * det(J_phi) = det(J_phi)^2 * det(eta_E)."
)
print(
    "If det(J_phi) = 1, then det(g_M) = det(eta_E) = -1. This means a purely additive INN phi is volume-preserving."
)
print(
    "The off-diagonal terms in g_M will indicate how phi shears or mixes the conceptual coordinates."
)
print(
    "The diagonal terms will show scaling. If g_M is not eta_E, then phi has introduced non-trivial geometry even if volume is preserved."
)
print(
    "To get the 'true' manifold structure, we should analyze g_M at m* = phi_inverse(e_final). For INNs, phi_inverse is exact."
)
...
if final_e_coords_inn:
    ...
    for token_id, e_final_np in final_e_coords_inn.items():
        ...
        with torch.no_grad():
            m_star_tensor = trained_inn_phi(
                e_final_tensor.unsqueeze(0), reverse=True
            ).squeeze(0)
        final_m_coords_from_inn_inverse[token_id] = m_star_tensor.numpy()
    plot_coords_map(
        final_m_coords_from_inn_inverse,
        "Learned Conceptual Manifold (m* = phi_inv(e_final)) via INN",
    )
    ...
    for token_id in points_to_analyze_inn:  # Analyze for the same first few tokens
        if token_id in final_m_coords_from_inn_inverse:
            ...
            def phi_for_jacobian_inn_final(
                m_input_tensor,
            ):  # Ensure fresh definition for clarity
                return trained_inn_phi(
                    m_input_tensor.unsqueeze(0), reverse=False
                ).squeeze(0)

            try:
                J_phi_at_m_star_final = torch.autograd.functional.jacobian(
                    phi_for_jacobian_inn_final, m_star_coord_torch
                )
                g_M_at_m_star_final = (
                    J_phi_at_m_star_final.T @ eta_E_torch_inn @ J_phi_at_m_star_final
                )
                ...
                print(
                    f"Error calculating Jacobian or g_M for '{token_id}' (final m*): {e_jac_inn_final}"
                )
else:
...
if (
    "trained_inn_phi" not in locals()
    or "final_m_coords_on_manifold_map" not in locals()
):
    print("Error: `trained_inn_phi` or `final_m_coords_on_manifold_map` not found.")
    print(
        "Please ensure these variables are defined from the INN training and inversion steps."
    )
    ...
    class DummyPhi(torch.nn.Module):
        ...
        def forward(self, x, reverse=False):
            return self.fc(x) if not reverse else x  # very basic
    ...
    if "final_m_coords_on_manifold_map" not in locals():
        final_m_coords_on_manifold_map = {
            "universe": np.array([0.0, 0.1, 0.1, 0.1]),
            "galaxy": np.array([0.1, 0.2, 0.2, 0.2]),
        }
        print("Using dummy final_m_coords_on_manifold_map for demonstration.")
        tokens_inn = list(
            final_m_coords_on_manifold_map.keys()
        )  # ensure tokens_inn matches
...
def analyze_local_geometry(m_coord_np, phi_network_model, eta_E):
    ...
    spatial_volume_factor = (
        np.sqrt(np.abs(det_g_spatial)) if det_g_spatial > 0 else np.nan
    )  # Proper if spatial part is Riemannian

    # Time-Space Mixing: sum of absolute values of g_0k (k=1,2,3)
    time_space_mixing = np.sum(np.abs(g_M_np[0, 1:])) + np.sum(
        np.abs(g_M_np[1:, 0])
    )  # Symmetric
...
if final_m_coords_on_manifold_map:
    ...
    tokens_to_analyze = (
        tokens_inn  # Or choose specific ones like ['universe', 'earth', 'sun']
    )
    ...
    for token_id in tokens_to_analyze:
        if token_id in final_m_coords_on_manifold_map:
            ...
            try:
                features = analyze_local_geometry(
                    m_star_coord_np, trained_inn_phi, eta_E_torch
                )
                ...
                print(
                    f"  sqrt(|g_tt|) (Time Distortion Factor): {features['time_distortion_sqrt_abs_g_tt']:.3f}"
                )
                print(
                    f"  Avg Spatial Diagonal Scaling (g_ii): {features['avg_spatial_diagonal_scaling']:.3f}"
                )
                print(
                    f"  sqrt(det(g_spatial)) (Spatial Volume Factor): {features['spatial_volume_sqrt_det_g_spatial']:.3f}"
                )
                print(
                    f"  Time-Space Mixing (sum |g_0k|): {features['time_space_mixing_abs_sum_g0k']:.3f}"
                )
                ...
                print(
                    f"Could not analyze geometry for '{token_id}' at m*={m_star_coord_np.round(3)}. Error: {e_jac}"
                )
...
print(
    " - If g_tt is consistently positive and large (as in your previous INN output example):"
)
print(
    "   This indicates your learned `phi` is strongly mapping the input `m_t` to influence spatial components in `E` OR"
)
print(
    "   the spatial `m_s` inputs strongly influence `e_t`. The 'time' dimension on M behaves spatially."
)
print(
    "   This could mean your hierarchy is primarily spatial in its learned representation, or the loss function"
)
print(
    "   forced `phi` into this configuration to satisfy constraints. You may need to regularize g_tt towards -1."
)
print(" - If g_tt is negative but |g_tt| != 1:")
print("   |g_tt| < 1: Proper time on M flows 'faster' than coordinate m_t.")
print(
    "   |g_tt| > 1: Proper time on M flows 'slower' than coordinate m_t (conceptual time dilation)."
)
...
print(
    "   Indicates that the conceptual time axis and spatial axes are not orthogonal on M."
)
print(
    "   'Movement' in conceptual space affects the 'flow' of conceptual time, and vice-versa."
)
print(" - Determinant of g_M close to -1 (for INN with additive coupling):")
print(
    "   Confirms volume preservation. If it deviates significantly with other INN types, it indicates volume distortion."
)
print(
...
print(
    "--- Phase 3 (Refined): Learning Diffeomorphism phi with Metric Regularization ---"
)
...
def train_inn_phi_embedding_with_metric_reg(
    ...
    for child_id, parent_id in hierarchy_pairs:
        if (
            parent_id is not None
            and child_id in initial_m_coords_tensors_map
            and parent_id in initial_m_coords_tensors_map
        ):
    ...
    if not training_pairs_ids:
        ...
        with torch.no_grad():
            for name, m_tensor in initial_m_coords_tensors_map.items():
                final_e_coords_map_on_abort[name] = (
                    inn_phi_model(m_tensor.unsqueeze(0)).squeeze(0).cpu().numpy()
                )
        return initial_m_coords_map, final_e_coords_map_on_abort, inn_phi_model

    print(
        f"Starting training of INN_phi with metric regularization for {num_epochs} epochs..."
    )
    for epoch in range(num_epochs):
        inn_phi_model.train()
        total_loss = 0.0
        ...
        for child_id, parent_id in training_pairs_ids:
            ...
            e_coords_child = inn_phi_model(m_coords_child_tensor.unsqueeze(0)).squeeze(
                0
            )
            e_coords_parent = inn_phi_model(
                m_coords_parent_tensor.unsqueeze(0)
            ).squeeze(0)
            ...
            J_phi_child = F.jacobian(
                lambda x: inn_phi_model(x.unsqueeze(0)).squeeze(0),
                m_coords_child_tensor,
            )
            g_M_child = J_phi_child.T @ eta_E_torch_for_loss @ J_phi_child

            # Jacobian and g_M for parent
            J_phi_parent = F.jacobian(
                lambda x: inn_phi_model(x.unsqueeze(0)).squeeze(0),
                m_coords_parent_tensor,
            )
            g_M_parent = J_phi_parent.T @ eta_E_torch_for_loss @ J_phi_parent

            # g_tt regularization
            if g_tt_reg_weight > 0:
                loss_g_tt_c = mse_loss_fn(
                    g_M_child[0, 0], torch.tensor(g_tt_target_value, device=device)
                )
                loss_g_tt_p = mse_loss_fn(
                    g_M_parent[0, 0], torch.tensor(g_tt_target_value, device=device)
                )
            ...
            if g_spatial_diag_reg_weight > 0:
                target_spatial_diagonals = torch.tensor(
                    [g_spatial_diag_target_value] * (inn_phi_model.input_dim - 1),
                    device=device,
                )
                loss_g_sp_diag_c = mse_loss_fn(
                    torch.diag(g_M_child[1:, 1:]), target_spatial_diagonals
                )
                loss_g_sp_diag_p = mse_loss_fn(
                    torch.diag(g_M_parent[1:, 1:]), target_spatial_diagonals
                )
                loss_metric_reg += g_spatial_diag_reg_weight * (
                    loss_g_sp_diag_c + loss_g_sp_diag_p
                )
            ...
            if g_off_diag_reg_weight > 0:
                # For g_M_child
                off_diag_child_sum_sq = (
                    torch.sum(g_M_child[0, 1:] ** 2)
                    + torch.sum(g_M_child[1:, 0] ** 2)
                    + torch.sum(torch.triu(g_M_child[1:, 1:], diagonal=1) ** 2)
                    + torch.sum(torch.tril(g_M_child[1:, 1:], diagonal=-1) ** 2)
                )
                # For g_M_parent
                off_diag_parent_sum_sq = (
                    torch.sum(g_M_parent[0, 1:] ** 2)
                    + torch.sum(g_M_parent[1:, 0] ** 2)
                    + torch.sum(torch.triu(g_M_parent[1:, 1:], diagonal=1) ** 2)
                    + torch.sum(torch.tril(g_M_parent[1:, 1:], diagonal=-1) ** 2)
                )
                loss_g_off_diag = mse_loss_fn(
                    off_diag_child_sum_sq + off_diag_parent_sum_sq,
        ...
        avg_g_tt = (
            total_g_tt_loss / (len(training_pairs_ids) * 2)
            if g_tt_reg_weight > 0
            else 0
        )
        avg_g_spatial = (
            total_g_spatial_loss / (len(training_pairs_ids) * 2)
            if g_spatial_diag_reg_weight > 0
            else 0
        )
        avg_g_off_diag = (
            total_g_off_diag_loss / len(training_pairs_ids)
            if g_off_diag_reg_weight > 0
            else 0
        )
        ...
        if epoch % (num_epochs // 10 or 1) == 0:
            print(
                f"Epoch {epoch + 1}/{num_epochs}, Avg Total Loss: {avg_loss:.4f} "
...
initial_m_coords_reg, final_e_coords_reg, trained_inn_phi_reg = (
    train_inn_phi_embedding_with_metric_reg(
        tokens_inn_reg,
        hierarchy_pairs_inn_reg,
        inn_phi_network_reg,
...
if final_e_coords_reg:
    ...
    for token_id, e_final_np in final_e_coords_reg.items():
        e_final_tensor = torch.tensor(e_final_np, dtype=torch.float32).to(
            next(trained_inn_phi_reg.parameters()).device
        )  # Ensure on same device
        with torch.no_grad():
            m_star_tensor = trained_inn_phi_reg(
                e_final_tensor.unsqueeze(0), reverse=True
            ).squeeze(0)
    ...
    if "plot_m_embedding" in locals() and "hierarchy_pairs_inn_reg" in locals():
        plot_m_embedding(
            final_m_coords_from_inn_inverse_reg,
            "Learned Conceptual Manifold (m* = phi_inv(e_final)) with Metric Reg",
        )
    else:
        print(
            "plot_m_embedding or hierarchy_pairs_inn_reg not defined, skipping m* plot."
        )
    ...
    for token_id in points_to_analyze_reg:
        if token_id in final_m_coords_from_inn_inverse_reg:
            ...
            m_star_coord_torch = torch.tensor(m_star_coord_np, dtype=torch.float32).to(
                next(trained_inn_phi_reg.parameters()).device
            )

            def phi_for_jacobian_inn_final_reg(m_input_tensor):
                return trained_inn_phi_reg(
                    m_input_tensor.unsqueeze(0), reverse=False
                ).squeeze(0)

            try:
                J_phi_at_m_star_final = F.jacobian(
                    phi_for_jacobian_inn_final_reg, m_star_coord_torch
                )
                # Ensure eta_E_torch_for_loss is on the same device as J_phi
                g_M_at_m_star_final = (
                    J_phi_at_m_star_final.T
                    @ eta_E_torch_for_loss.to(J_phi_at_m_star_final.device)
                    @ J_phi_at_m_star_final
                )
                ...
                print(
                    f"Error calculating Jacobian or g_M for '{token_id}' (final m* with reg): {e_jac_inn_final}"
                )
...
print(
    " - g_tt components of the pullback metric (at final m* coordinates) to be closer to -1.0."
)
print(" - Spatial diagonal components (g_xx, g_yy, g_zz) to be closer to 1.0.")
print(" - Off-diagonal components to be closer to 0.0.")
print(
    " - The determinant of g_M should remain close to -1.0 due to the volume-preserving nature of additive INNs."
)
print(
    "The training loss components for these regularizations should decrease over epochs if effective."
)
...
def train_inn_phi_embedding_with_metric_reg_v2(  # Renamed for clarity
    ...
    initial_m_coords_map = {
        token_id: np.array(
            [0.0] + np.random.uniform(-0.5, 0.5, inn_phi_model.input_dim - 1).tolist(),
            dtype=np.float32,
        )  # Smaller initial range
        for token_id in tokens
    }
    initial_m_coords_tensors_map = {
        name: torch.tensor(coords, dtype=torch.float32, device=device)
    ...
    print(
        f"Starting training of INN_phi with MODIFIED metric regularization for {num_epochs} epochs..."
    )
    print(
        f"LR={learning_rate}, g_tt_w={g_tt_reg_weight}, g_sp_w={g_spatial_diag_reg_weight}, g_off_w={g_off_diag_reg_weight}, clip={gradient_clip_value}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for child_id, parent_id in training_pairs_ids:
            ...
            for m_point_tensor in points_for_reg:
                ...
                J_phi = F.jacobian(
                    lambda x: inn_phi_model(x.unsqueeze(0)).squeeze(0),
                    m_point_tensor.detach().clone().requires_grad_(True),
                )
            ...
            if g_tt_reg_weight > 0:
                ...
                loss_g_tt_mse_c = mse_loss_fn(
                    g_M_child[0, 0], torch.tensor(g_tt_target_value, device=device)
                )
                loss_g_tt_mse_p = mse_loss_fn(
                    g_M_parent[0, 0], torch.tensor(g_tt_target_value, device=device)
                )
                ...
                push_negative_c = torch.relu(
                    g_M_child[0, 0] - torch.tensor(g_tt_target_value, device=device)
                )
                push_negative_p = torch.relu(
                    g_M_parent[0, 0] - torch.tensor(g_tt_target_value, device=device)
                )
                ...
                current_g_tt_loss = g_tt_reg_weight * (
                    loss_g_tt_mse_c
                    + loss_g_tt_mse_p  # Push towards target value
                    + (push_negative_c + push_negative_p)
                    * 10.0  # Extra penalty for being too high (e.g., positive)
                )

                loss_metric_reg += current_g_tt_loss

            if g_spatial_diag_reg_weight > 0:
                target_spatial_diagonals = torch.full(
                    (inn_phi_model.input_dim - 1,),
                    g_spatial_diag_target_value,
                    device=device,
                )
                loss_g_sp_diag_c = mse_loss_fn(
                    torch.diag(g_M_child[1:, 1:]), target_spatial_diagonals
                ...
                current_g_spatial_loss = g_spatial_diag_reg_weight * (
                    loss_g_sp_diag_c + loss_g_sp_diag_p
                )
            ...
            if g_off_diag_reg_weight > 0:
                ...
                g_M_child_offdiag.diagonal().fill_(
                    0
                )  # Zero out diagonal to sum only off-diagonals
                loss_g_off_diag_c = mse_loss_fn(
                    g_M_child_offdiag, torch.zeros_like(g_M_child_offdiag)
                )

                g_M_parent_offdiag = g_M_parent.clone()
                g_M_parent_offdiag.diagonal().fill_(0)
                loss_g_off_diag_p = mse_loss_fn(
                    g_M_parent_offdiag, torch.zeros_like(g_M_parent_offdiag)
                )

                current_g_off_diag_loss = g_off_diag_reg_weight * (
                    loss_g_off_diag_c + loss_g_off_diag_p
                )
            ...
            epoch_losses["g_tt"].append(
                current_g_tt_loss.item()
                if isinstance(current_g_tt_loss, torch.Tensor)
                else current_g_tt_loss
            )
            epoch_losses["g_spatial"].append(
                current_g_spatial_loss.item()
                if isinstance(current_g_spatial_loss, torch.Tensor)
                else current_g_spatial_loss
            )
            epoch_losses["g_off_diag"].append(
                current_g_off_diag_loss.item()
                if isinstance(current_g_off_diag_loss, torch.Tensor)
                else current_g_off_diag_loss
            )
        ...
        if epoch % (num_epochs // 20 or 1) == 0 or epoch == num_epochs - 1:
            print(
                f"Epoch {epoch + 1}/{num_epochs}, Avg Total Loss: {avg_total_loss:.4f} "
        ...
        if (
            epochs_no_improve >= patience and epoch > num_epochs // 4
        ):  # Only activate early stopping after some initial training
            print(
                f"Early stopping at epoch {epoch + 1} due to no improvement in total loss for {patience} epochs."
            )
            break
...
inn_phi_network_reg_v2 = INNPhi(
    input_dim=4, hidden_dim=128, num_coupling_layers=8
)  # Potentially deeper network

print("\n--- Training INN Phi Network with Adjusted Metric Regularization ---")
initial_m_coords_reg_v2, final_e_coords_reg_v2, trained_inn_phi_reg_v2 = (
    train_inn_phi_embedding_with_metric_reg_v2(
        tokens_inn_reg_v2,
        hierarchy_pairs_inn_reg_v2,
...
if final_e_coords_reg_v2:
    ...
    for token_id, e_final_np in final_e_coords_reg_v2.items():
        e_final_tensor = torch.tensor(e_final_np, dtype=torch.float32).to(
            next(trained_inn_phi_reg_v2.parameters()).device
        )
        with torch.no_grad():
            m_star_tensor = trained_inn_phi_reg_v2(
                e_final_tensor.unsqueeze(0), reverse=True
            ).squeeze(0)
    ...
    if "plot_m_embedding" in locals() and "hierarchy_pairs_inn_reg_v2" in locals():
        plot_m_embedding(
            final_m_coords_from_inn_inverse_reg_v2,
            "Learned Conceptual Manifold (m* = phi_inv(e_final)) with Adjusted Metric Reg",
        )
    else:
        print(
            "plot_m_embedding or hierarchy_pairs_inn_reg_v2 not defined, skipping m* plot."
        )
    ...
    for token_id in points_to_analyze_reg_v2:
        if token_id in final_m_coords_from_inn_inverse_reg_v2:
            ...
            def phi_for_jacobian_inn_final_reg_v2(m_input_tensor):
                return trained_inn_phi_reg_v2(
                    m_input_tensor.unsqueeze(0), reverse=False
                ).squeeze(0)

            try:
                J_phi_at_m_star_final = F.jacobian(
                    phi_for_jacobian_inn_final_reg_v2, m_star_coord_torch
                )
                g_M_at_m_star_final = (
                    J_phi_at_m_star_final.T
                    @ eta_E_torch_for_loss.to(J_phi_at_m_star_final.device)
                ...
                print(
                    f"Error calculating Jacobian or g_M for '{token_id}' (final m* with adjusted reg): {e_jac_inn_final}"
                )
else:
    print(
        "final_e_coords_reg_v2 is empty. Cannot calculate final m_coords or their g_M."
    )
...
def train_inn_phi_embedding_with_metric_reg_v3(
    ...
    print(
        f"Starting training of INN_phi with MODIFIED metric regularization (v3) for {num_epochs} epochs..."
    )
    print(f"LR={learning_rate}, eps1_sq={epsilon1_target_sq}, causal_m={causal_margin}")
    print(
        f"g_tt_w={g_tt_reg_weight}, g_sp_diag_w={g_spatial_diag_reg_weight}, g_sp_pos_w={g_spatial_positivity_weight}, g_off_diag_w={g_off_diag_reg_weight}, clip={gradient_clip_value}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        def get_g_M(m_tensor_for_jac):
            ...
            m_tensor_for_jac_grad = (
                m_tensor_for_jac.detach().clone().requires_grad_(True)
            )
            J_phi = F.jacobian(
                lambda x: inn_phi_model(x.unsqueeze(0)).squeeze(0),
                m_tensor_for_jac_grad,
        ...
        for child_id, parent_id in training_pairs_ids:
            ...
            e_coords_child = inn_phi_model(
                m_coords_child_tensor_input.unsqueeze(0)
            ).squeeze(0)
            e_coords_parent = inn_phi_model(
                m_coords_parent_tensor_input.unsqueeze(0)
            ).squeeze(0)
            ...
            if g_tt_reg_weight > 0:
                ...
                loss_g_tt_mse = mse_loss_fn(
                    g_tt_c, torch.tensor(g_tt_target_value, device=device)
                ...
                loss_g_tt_pos_penalty = torch.relu(
                    g_tt_c - g_tt_target_value
                ) + torch.relu(g_tt_p - g_tt_target_value)

                step_g_tt_loss = g_tt_reg_weight * (
                    loss_g_tt_mse + loss_g_tt_pos_penalty * 10.0
                )  # x10 emphasis
            ...
            if g_spatial_diag_reg_weight > 0:
                ...
                loss_g_sp_diag = mse_loss_fn(
                    spatial_diagonals_child, target_spatial_diagonals
            ...
            if g_spatial_positivity_weight > 0:
                ...
                step_g_sp_pos_loss = g_spatial_positivity_weight * (
                    loss_g_sp_pos_c + loss_g_sp_pos_p
                )
            ...
            if g_off_diag_reg_weight > 0:
                g_M_child_offdiag = g_M_child.clone()
                g_M_child_offdiag.diagonal().fill_(0)
                g_M_parent_offdiag = g_M_parent.clone()
                g_M_parent_offdiag.diagonal().fill_(0)
                loss_g_off_diag = mse_loss_fn(
                    g_M_child_offdiag, torch.zeros_like(g_M_child_offdiag)
...
initial_m_coords_reg_v3, final_e_coords_reg_v3, trained_inn_phi_reg_v3 = (
    train_inn_phi_embedding_with_metric_reg_v3(
        tokens_inn_reg_v3,
        hierarchy_pairs_inn_reg_v3,
...
if final_e_coords_reg_v3 and "universe" in final_e_coords_reg_v3:
    ...
    e_final_univ_tensor = torch.tensor(e_final_univ_np, dtype=torch.float32).to(
        next(trained_inn_phi_reg_v3.parameters()).device
    )
    with torch.no_grad():
        m_star_univ_tensor = trained_inn_phi_reg_v3(
            e_final_univ_tensor.unsqueeze(0), reverse=True
        ).squeeze(0)
    m_star_univ_np = m_star_univ_tensor.cpu().numpy()

    if (
        "analyze_local_geometry" in locals()
    ):  # Check if the analysis function is defined
        univ_geo_features = analyze_local_geometry(
            m_star_univ_np,
            trained_inn_phi_reg_v3,
            eta_E_torch_for_loss.to(next(trained_inn_phi_reg_v3.parameters()).device),
        )
        ...
        print(
            f"Avg Spatial Diag: {univ_geo_features['avg_spatial_diagonal_scaling']:.3f}"
        )
        print(
            f"Time-Space Mixing: {univ_geo_features['time_space_mixing_abs_sum_g0k']:.3f}"
        )
    ...
    for token_id, e_final_np_plot in final_e_coords_reg_v3.items():
        e_final_tensor_plot = torch.tensor(e_final_np_plot, dtype=torch.float32).to(
            next(trained_inn_phi_reg_v3.parameters()).device
        )
        with torch.no_grad():
            m_star_tensor_plot = trained_inn_phi_reg_v3(
                e_final_tensor_plot.unsqueeze(0), reverse=True
            ).squeeze(0)
        final_m_coords_from_inn_inverse_reg_v3[token_id] = (
            m_star_tensor_plot.cpu().numpy()
        )

    if "plot_m_embedding" in locals() and "hierarchy_pairs_inn_reg_v3" in locals():
        plot_m_embedding(
            final_m_coords_from_inn_inverse_reg_v3,
            "Learned Conceptual Manifold (m* = phi_inv(e_final)) with v3 Metric Reg",
        )
...
print(
    "--- Phase 3 (Revised with clearer Loss & Debugging v4): Learning Diffeomorphism phi ---"
)
...
def train_inn_phi_embedding_with_metric_reg_v4(
    ...
    for i, token_id in enumerate(tokens):
        ...
        initial_m_coords_map[token_id] = np.array(
            [current_t]
            + np.random.uniform(
                -0.1, 0.1, inn_phi_model.input_dim - 1
            ).tolist(),  # Smaller spatial range
            dtype=np.float32,
        )
    ...
    # Print all weights
    print(
        f"LR={learning_rate}, causal_w={causal_loss_weight}, interval_w={interval_loss_weight}"
    )
    print(f"g_tt_mse_w={g_tt_mse_weight}, g_tt_sign_w={g_tt_sign_penalty_weight}")
    print(
        f"g_sp_diag_mse_w={g_spatial_diag_mse_weight}, g_sp_pos_w={g_spatial_positivity_penalty_weight}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for child_id, parent_id in training_pairs_ids:
            ...
            delta_e_t = e_coords_child[0] - e_coords_parent[0]
            loss_causal_order = torch.relu(
                -(delta_e_t - causal_margin)
            )  # Target: delta_e_t > causal_margin
            ...
            ds_sq_E = -(delta_e_t**2) + delta_e_spatial_sq
            loss_interval = (
                ds_sq_E - (-epsilon1_target_sq)
            ) ** 2  # Target: ds_sq_E = -epsilon1_target_sq

            # --- Metric Regularization Losses ---
            loss_metric_reg = torch.tensor(0.0, device=device)

            # Calculate Jacobians and g_M for child and parent
            g_M_child = get_g_M_for_loss(
                m_coords_child_tensor_input, inn_phi_model, eta_E_torch_for_loss
            )
            g_M_parent = get_g_M_for_loss(
                m_coords_parent_tensor_input, inn_phi_model, eta_E_torch_for_loss
            )

            # g_tt regularization
            g_tt_c, g_tt_p = g_M_child[0, 0], g_M_parent[0, 0]
            loss_g_tt_m = mse_loss_fn(
                g_tt_c, torch.tensor(g_tt_target, device=device)
            ...
            step_g_tt_loss = (
                g_tt_mse_weight * loss_g_tt_m + g_tt_sign_penalty_weight * loss_g_tt_s
            )
            loss_metric_reg += step_g_tt_loss
            ...
            spatial_diagonals_parent = torch.diag(g_M_parent[1:, 1:])
            target_spatial_diagonals = torch.full(
                (inn_phi_model.input_dim - 1,), g_spatial_diag_target, device=device
            )

            loss_g_sp_diag_m = mse_loss_fn(
                spatial_diagonals_child, target_spatial_diagonals
            ) + mse_loss_fn(spatial_diagonals_parent, target_spatial_diagonals)
            # Penalize if any g_ii (spatial) < 0
            loss_g_sp_pos = torch.sum(torch.relu(-spatial_diagonals_child)) + torch.sum(
                torch.relu(-spatial_diagonals_parent)
            )
            step_g_spatial_loss = (
                g_spatial_diag_mse_weight * loss_g_sp_diag_m
            ...
            loss_g_off_diag_m = mse_loss_fn(
                g_M_child_offdiag, target_off_diag
            ...
            # Total Loss
            loss = (
                causal_loss_weight * loss_causal_order
            ...
            epoch_losses_agg["causal"].append(
                causal_loss_weight * loss_causal_order.item()
            )
            epoch_losses_agg["interval"].append(
                interval_loss_weight * loss_interval.item()
            )
            epoch_losses_agg["g_tt"].append(step_g_tt_loss.item())
            epoch_losses_agg["g_spatial_diag"].append(
                g_spatial_diag_mse_weight * loss_g_sp_diag_m.item()
            )  # Log only MSE part
            epoch_losses_agg["g_spatial_pos"].append(
                g_spatial_positivity_penalty_weight * loss_g_sp_pos.item()
            )
        ...
        if epoch % log_interval == 0 or epoch == num_epochs - 1:
            ...
            if epoch % (log_interval * 5) == 0 and len(tokens) > 0:  # Less frequent
                with torch.no_grad():
                    ...
                    g_M_sample = get_g_M_for_loss(
                        sample_m_tensor, inn_phi_model, eta_E_torch_for_loss
                    )
                    print(
                        f"  Sample g_M for '{sample_token_id}' (g_tt: {g_M_sample[0, 0].item():.3f}, "
        ...
        if (
            epochs_no_improve >= patience and epoch > num_epochs // 2
        ):  # Stricter early stopping
            print(
                f"Early stopping at epoch {epoch + 1} as total loss hasn't improved for {patience} epochs."
            )
            break
...
inn_phi_network_reg_v4 = INNPhi(
    input_dim=4, hidden_dim=64, num_coupling_layers=6
)  # Start with a slightly smaller network
...
initial_m_coords_reg_v4, final_e_coords_reg_v4, trained_inn_phi_reg_v4 = (
    train_inn_phi_embedding_with_metric_reg_v4(
        tokens_inn_reg_v4,
        hierarchy_pairs_inn_reg_v4,
...
if final_e_coords_reg_v4 and "universe" in final_e_coords_reg_v4:
    ...
    e_final_univ_tensor_v4 = torch.tensor(e_final_univ_np_v4, dtype=torch.float32).to(
        next(trained_inn_phi_reg_v4.parameters()).device
    )
    with torch.no_grad():
        m_star_univ_tensor_v4 = trained_inn_phi_reg_v4(
            e_final_univ_tensor_v4.unsqueeze(0), reverse=True
        ).squeeze(0)
    m_star_univ_np_v4 = m_star_univ_tensor_v4.cpu().numpy()

    if "analyze_local_geometry" in locals():
        univ_geo_features_v4 = analyze_local_geometry(
            m_star_univ_np_v4,
            trained_inn_phi_reg_v4,
            eta_E_torch_for_loss.to(next(trained_inn_phi_reg_v4.parameters()).device),
        )
        print(f"g_M at 'universe' (final m*): \n{univ_geo_features_v4['g_M'].round(3)}")
        print(f"det(g_M): {univ_geo_features_v4['det_g_M']:.3e}")
        print(f"g_tt: {univ_geo_features_v4['g_tt']:.3f}")
        print(
            f"Avg Spatial Diag: {univ_geo_features_v4['avg_spatial_diagonal_scaling']:.3f}"
        )
        print(
            f"Time-Space Mixing: {univ_geo_features_v4['time_space_mixing_abs_sum_g0k']:.3f}"
        )
    ...
    for token_id, e_final_np_plot in final_e_coords_reg_v4.items():
        ...
        with torch.no_grad():
            m_star_tensor_plot = trained_inn_phi_reg_v4(
                e_final_tensor_plot.unsqueeze(0), reverse=True
            ).squeeze(0)
        final_m_coords_from_inn_inverse_reg_v4[token_id] = (
            m_star_tensor_plot.cpu().numpy()
        )
    ...
    if "plot_m_embedding" in locals() and "hierarchy_pairs_inn_reg_v4" in locals():
        plot_m_embedding(
            final_m_coords_from_inn_inverse_reg_v4,
            "Learned Conceptual Manifold (m* = phi_inv(e_final)) with v4 Metric Reg",
        )


print("\n--- This v4 training aims for more stability and better metric signature. ---")
print(
    "--- Monitor loss components closely during training and adjust weights/LR iteratively. ---"
)
...
def plot_m_embedding_with_g_feature(
    m_coords_map_plot,
    hierarchy_pairs_plot,
    feature_map,
    feature_name,
...
eta_E_torch = torch.tensor(np.diag([-1.0, 1.0, 1.0, 1.0]), dtype=torch.float32).to(
    next(trained_inn_phi.parameters()).device
)
...
if final_e_coords_reg_v4:
    ...
    print(
        "`final_e_coords_reg_v4` is empty. Cannot proceed with m* calculation and analysis."
    )
...
for child, parent in hierarchy_pairs_inn_reg_v4:
    if parent is not None:  # Exclude root's "None" parent
        if (
            child in tokens_inn_reg_v4 and parent in tokens_inn_reg_v4
        ):  # Ensure nodes are in our token list
            out_degree[parent] += 1
            in_degree[child] += 1
...
if final_m_coords_on_manifold_map:
    for token_id, m_star_coord_np in final_m_coords_on_manifold_map.items():
        try:
            geo_features = analyze_local_geometry(
                m_star_coord_np, trained_inn_phi, eta_E_torch
            )
            data_for_df.append(
                {
...
# --- 3. Basic Correlation ---
if (
    not df_analysis.empty and len(df_analysis) > 1
):  # Correlation needs at least 2 data points
...
def plot_m_embedding_with_paths(
    m_coords_map_plot,
    hierarchy_pairs_plot,
    final_e_coords_plot,
    phi_inverse_model,
    ...
    paths_to_plot=None,
):  # Optional: list of (parent, child) tuples
    ...
    for parent_id_path, child_id_path in paths_to_plot:
        if (
            parent_id_path in final_e_coords_plot
            and child_id_path in final_e_coords_plot
        ):
            e_parent = torch.tensor(
                final_e_coords_plot[parent_id_path],
                dtype=torch.float32,
                device=device_paths,
            )
            e_child = torch.tensor(
                final_e_coords_plot[child_id_path],
                dtype=torch.float32,
                device=device_paths,
            )

            e_path_points = torch.stack(
                [
                    e_parent + (e_child - e_parent) * t
...
if final_m_coords_on_manifold_map and final_e_coords_reg_v4:
    ...
    for p, c in paths_to_visualize:
        ...
        if found:
            ...
            print(
                f"Path {p}->{c} not in hierarchy_pairs, skipping for path visualization."
            )
...
print(
    "The solid colored lines in the plot are the preimages (under phi_inverse) of straight lines in the E-space."
)
print(
    "These represent the 'straightest possible paths' (geodesics) on your learned conceptual manifold M."
)
print(
    "Their curvature visually demonstrates the non-Euclidean geometry induced by your learned phi."
)
print(
    "The dotted gray lines are the direct connections in m-coordinate space, highlighting the difference."
)
...
if "df_analysis" not in locals() or df_analysis.empty:
    ...
    print(
        "Please ensure the previous cell that creates and populates df_analysis has been run successfully."
    )
...
if (
    ...
    and "m_x" in df_analysis.columns
    and "g_tt" in df_analysis.columns
):
    plt.figure(figsize=(10, 6))
    sns.scatterplot(
        data=df_analysis, x="m_x", y="g_tt", hue="token", s=100, legend=False
    )  # Use token for hue temporarily
    ...
    if np.abs(correlation_mx_gtt) > 0.7:
        ...
        print(
            "The linear relationship is weak or non-existent. The visual trend might be non-linear."
        )
...
print(
    "3.  **Embedding Density:** Hubs are connected to many other nodes. To fit all these 'almost null' links in the E-space without violating causality, the mapping $\phi$ might need to 'bend' the M-space more significantly in the vicinity of these hubs. This is analogous to how massive objects warp spacetime more strongly."
)

print("\nHow to Quantify Path Curvature (Conceptual Approaches):")
print(
    "Directly quantifying the 'curvature' of a plotted 3D line is non-trivial. Here are some ideas:"
)
print(
    "  a.  **Deviation from Straight Line in M-space:** For each path (preimage of straight E-space line), calculate the sum of squared distances of intermediate path points from the direct straight line connecting the start and end m*-coordinates. Higher values = more 'bent'."
)
print(
    "  b.  **Integrated Curvature along Path (Advanced):** If you had the full Riemann curvature tensor derived from $g_{\mathcal{M}}$, you could integrate some measure of curvature (e.g., components of Riemann tensor projected onto the path's tangent vectors) along the path."
)
print(
    "  c.  **Change in Tangent Vectors:** Calculate the angle change between successive tangent vectors along the discretized path in M-space. A larger total angle change indicates more bending."
)
print(
    "  d.  **Approximation using Local $g_{\mathcal{M}}$ Variation:** For a path from $m_A^*$ to $m_B^*$, look at how much $g_{\mathcal{M}}$ (e.g., its determinant or key components) varies for points $m^*$ along this path compared to $g_{\mathcal{M}}(m_A^*)$ and $g_{\mathcal{M}}(m_B^*)$. Rapid changes in $g_{\mathcal{M}}$ along the path suggest significant underlying curvature affecting the path's shape."
)

print("\nSteps to Test Your Hypothesis:")
print(
    "1.  **Define a Path Curvature Metric:** Choose one of the methods above (e.g., start with 'Deviation from Straight Line in M-space' as it's simpler)."
)
print(
    "2.  **Calculate for All Hierarchical Paths:** For every parent-child link in your `hierarchy_pairs`:"
)
print("    i.   Get their `final_e_coords`.")
print(
    "    ii.  Generate the discretized path in M-space by interpolating in E-space and applying `phi_inverse`."
)
print("    iii. Calculate your chosen path curvature metric for this M-space path.")
print(
    "3.  **Aggregate Degree Information:** For each path (link), consider the `total_degree` of its parent node, its child node, or their average/sum."
)
print(
    "4.  **Correlate:** Plot and calculate the correlation between the path curvature metric and the aggregated degree information."
)
...
from transformers import (
    BertTokenizer,
    BertModel,
)  # Assuming these are for future use or context
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from collections import defaultdict
...
def get_wordnet_subtree(start_synset_name="mammal.n.01", depth_limit=3):
    ...
    try:
        ...
        print(
            f"Error: Could not find synset '{start_synset_name}'. Please check the name. ({e})"
        )
        return [], [], {}
    ...
    while queue:
        ...
        current_lemma_name = (
            current_synset.lemmas()[0].name().replace("_", " ")
            if current_synset.lemmas()
            else None
        )
    ...
    is_root_explicit = any(
        p is None for c, p in hierarchy_pairs_set if c == root_token_name
    )
    if not is_root_explicit and root_token_name in concepts_set:
        ...
        if not is_a_parent and depth_limit == 0:  # Single node extraction
            ...
            c == root_token_name and p is not None for c, p in hierarchy_pairs_set
        ):  # Not a child of anything IN THE SUBTREE
            hierarchy_pairs_set.add((root_token_name, None))
    ...
    for child, parent in hierarchy_list:
        if child in final_tokens_set and (parent is None or parent in final_tokens_set):
            ...
            child in final_tokens_set
            and parent is not None
            and parent not in final_tokens_set
        ):
...
tokens_wn, hierarchy_pairs_wn, concepts_details_wn = get_wordnet_subtree(
    start_synset_name=WN_START_SYNSET, depth_limit=WN_DEPTH_LIMIT
)

if not tokens_wn or not hierarchy_pairs_wn:
    print(
        f"Could not extract a valid hierarchy from WordNet starting with '{WN_START_SYNSET}' at depth {WN_DEPTH_LIMIT}."
    )
    print("Skipping further WordNet-based steps.")
else:
    print(
        f"Extracted {len(tokens_wn)} concepts and {len(hierarchy_pairs_wn)} hierarchy pairs from WordNet."
    )
    ...
    inn_phi_wn = INNPhi(
        input_dim=4, hidden_dim=128, num_coupling_layers=6
    ...
    initial_m_coords_wn, final_e_coords_wn, trained_inn_phi_wn = (
        train_inn_phi_embedding_with_metric_reg_v4(
            tokens_wn,
            hierarchy_pairs_wn,
            inn_phi_wn,
    ...
    if final_e_coords_wn:
        ...
        for token_id, e_final_np in final_e_coords_wn.items():
            ...
            with torch.no_grad():
                m_star_tensor = trained_inn_phi_wn(
                    e_final_tensor.unsqueeze(0), reverse=True
                ).squeeze(0)
    ...
    for token in tokens_wn:
        _ = total_degree_wn[token]  # Ensure all have entry

    # Create Pandas DataFrame
    data_for_df_wn = []
    if final_m_coords_wn_map:
        eta_E_torch_wn = torch.tensor(
            np.diag([-1.0, 1.0, 1.0, 1.0]), dtype=torch.float32
        ).to(device_wn_analysis)
        for token_id, m_star_coord_np in final_m_coords_wn_map.items():
            try:
                geo_features = analyze_local_geometry(
                    m_star_coord_np, trained_inn_phi_wn, eta_E_torch_wn
                )
                data_for_df_wn.append(
                    {
                        ...
                        "g_yy": geo_features["g_M"][2, 2],
                        "g_zz": geo_features["g_M"][3, 3],
                        "avg_g_spatial_diag": geo_features[
                            "avg_spatial_diagonal_scaling"
                        ],
                        "det_g_M": geo_features["det_g_M"],
                        "time_space_mix": geo_features["time_space_mixing_abs_sum_g0k"],
                        "depth": concepts_details_wn.get(token_id, {}).get("depth", -1),
                        "in_degree": in_degree_wn[token_id],
                        "out_degree": out_degree_wn[token_id],
                        "total_degree": total_degree_wn[token_id],
                    }
                )
            except Exception as e:
                print(
                    f"Could not analyze geometry for WN DataFrame for '{token_id}': {e}"
                )
    ...
    if not df_analysis_wn.empty:
        ...
        plt.title(
            "WordNet: Conceptual $m_x$ vs. $g_{tt}$ (Color by Depth, Size by Degree)"
        )
        plt.xlabel("Conceptual X-coordinate ($m_x$) on Manifold $\mathcal{M}$")
        plt.ylabel("$g_{tt}$ component of Pullback Metric $g_{\mathcal{M}}$")
        plt.axhline(
            -1.0, color="gray", linestyle="--", label="$g_{tt} = -1.0$ (Target)"
        )
        plt.grid(True, linestyle=":", alpha=0.7)
        ...
        for parent_id, child_id in hierarchy_pairs_wn:
            if (
                parent_id is None
                or child_id not in final_e_coords_wn
                or parent_id not in final_e_coords_wn
            ):
                continue
            e_parent = torch.tensor(
                final_e_coords_wn[parent_id],
                dtype=torch.float32,
                device=device_wn_analysis,
            )
            e_child = torch.tensor(
                final_e_coords_wn[child_id],
                dtype=torch.float32,
                device=device_wn_analysis,
            )
            e_path_points_tensor = torch.stack(
                [e_parent + (e_child - e_parent) * t for t in np.linspace(0, 1, 30)]
            )
            with torch.no_grad():
                m_path_points_tensor = trained_inn_phi_wn(
                    e_path_points_tensor, reverse=True
                )
            curvature_metric = calculate_spatial_path_deviation(
                m_path_points_tensor.cpu().numpy()
            )
            path_curvature_data_wn.append(
                {
                    ...
                    "avg_degree_of_pair": (
                        total_degree_wn[parent_id] + total_degree_wn[child_id]
                    )
                    / 2.0,
                    "max_degree_of_pair": max(
                        total_degree_wn[parent_id], total_degree_wn[child_id]
                    ),
                }
            )
        ...
        if not df_paths_wn.empty:
            ...
            plt.subplot(1, 2, 1)
            sns.scatterplot(
                data=df_paths_wn, x="avg_degree_of_pair", y="curvature_metric"
            )
            sns.regplot(
                data=df_paths_wn,
                x="avg_degree_of_pair",
                y="curvature_metric",
                scatter=False,
                color="red",
                order=2,
            ...
            plt.xlabel("Average Total Degree")
            plt.ylabel("Path Curvature Metric")
            plt.grid(True, linestyle=":", alpha=0.7)

            plt.subplot(1, 2, 2)
            sns.scatterplot(
                data=df_paths_wn, x="max_degree_of_pair", y="curvature_metric"
            )
            sns.regplot(
                data=df_paths_wn,
                x="max_degree_of_pair",
                y="curvature_metric",
                scatter=False,
                color="blue",
                order=2,
            ...
            plt.xlabel("Maximum Total Degree")
            plt.ylabel("Path Curvature Metric")
            plt.grid(True, linestyle=":", alpha=0.7)
            plt.tight_layout()
            plt.show()
            ...
            if len(df_paths_wn) > 1:
                print(
                    f"  vs. Avg Degree: {df_paths_wn['curvature_metric'].corr(df_paths_wn['avg_degree_of_pair']):.4f}"
                )
                print(
                    f"  vs. Max Degree: {df_paths_wn['curvature_metric'].corr(df_paths_wn['max_degree_of_pair']):.4f}"
                )
...
if (
    "trained_inn_phi_wn" not in locals()
    or "final_m_coords_wn_map" not in locals()
    or "df_paths_wn" not in locals()
):
    print(
        "Warning: WordNet specific trained model/data not found. Using generic dummy data for plotting."
    )
    ...
    if "trained_inn_phi" not in locals():  # If even the generic dummy is not there
        ...
        class DummyPhi(torch.nn.Module):
            ...
            def forward(self, x, reverse=False):
                return (
                    self.fc(x) + 0.1 * torch.sin(x)
                    if not reverse
                    else x - 0.1 * torch.sin(x)
                )

        trained_inn_phi_wn = DummyPhi()
    else:
        trained_inn_phi_wn = (
            trained_inn_phi  # Fallback to generic if WN specific one isn't there
        )
    ...
    if "final_m_coords_on_manifold_map" in locals() and final_m_coords_on_manifold_map:
        ...
        final_e_coords_wn = {
            k: trained_inn_phi_wn(torch.tensor(v, dtype=torch.float32).unsqueeze(0))
            .squeeze(0)
            .detach()
            .numpy()
            for k, v in final_m_coords_wn_map.items()
        }  # Reconstruct e_coords
    else:  # Absolute fallback
        _tokens_dummy_wn = ["mammal", "placental", "carnivore", "canine", "dog", "cat"]
        final_e_coords_wn = {
            "mammal": np.array([0, 0, 0, 0]),
            "placental": np.array([0.1, 0.5, 0, 0]),
            "carnivore": np.array([0.2, 0.5, 0.5, 0]),
            "canine": np.array([0.3, 1, 0.5, 0]),
            "dog": np.array([0.4, 1, 0.2, 0]),
            "cat": np.array([0.3, 0.2, 0.8, 0]),
        }
        with torch.no_grad():
            final_m_coords_wn_map = {
                k: trained_inn_phi_wn(
                    torch.tensor(v, dtype=torch.float32).unsqueeze(0), reverse=True
                )
                .squeeze(0)
                .numpy()
    ...
    if "df_paths_wn" not in locals() or df_paths_wn.empty:
        _dummy_path_data = [
            {"parent": p, "child": c, "curvature_metric": np.random.rand() * 0.0005}
            for c, p in hierarchy_pairs_wn
            if p is not None
        ]
...
if not df_paths_wn.empty:
    ...
    if len(df_paths_wn_sorted) >= 1:
        highlight_from_df.extend(
            list(
                zip(
                    df_paths_wn_sorted["parent"].head(3),
                    df_paths_wn_sorted["child"].head(3),
                )
            )
        )
        highlight_from_df.extend(
            list(
                zip(
                    df_paths_wn_sorted["parent"].tail(3),
                    df_paths_wn_sorted["child"].tail(3),
                )
            )
        )
...
if not df_paths_wn.empty:
    plotted_paths_df_subset = df_paths_wn[
        df_paths_wn.apply(
            lambda row: (row["parent"], row["child"]) in paths_to_highlight_wn
            and row["parent"] in valid_nodes_in_plot
            and row["child"] in valid_nodes_in_plot,
            axis=1,
        )
    ]
else:
...
if final_m_coords_wn_map and final_e_coords_wn and not plotted_paths_df_subset.empty:
    ...
    def plot_m_embedding_paths_colored_by_curvature_labeled(
        ...
        # Node scatter plot
        node_colors = [
            concepts_details_wn.get(token, {}).get("depth", 0) for token in labels_all
        ]  # Color by depth
        ...
        cbar_nodes = fig.colorbar(
            node_scatter, label="WordNet Depth", shrink=0.5, aspect=15, pad=0.15
        )
        ...
        if not path_data_df.empty and "curvature_metric" in path_data_df.columns:
            ...
            if len(curvatures) > 0 and not np.all(np.isnan(curvatures)):
                ...
                if (
                    min_curv == max_curv
                ):  # Avoid division by zero if all curvatures are same
                    norm = plt.Normalize(
                        vmin=min_curv - 0.00001, vmax=max_curv + 0.00001
                    )
                else:
                    norm = plt.Normalize(vmin=min_curv, vmax=max_curv)
                cmap_paths = plt.cm.get_cmap(
                    "autumn_r"
                )  # Use a different cmap for paths
                ...
                for idx_path, row_path in path_data_df.iterrows():
                    ...
                    if (
                        parent_id_p in final_e_coords_wn
                        and child_id_p in final_e_coords_wn
                    ):
                        e_parent = torch.tensor(
                            final_e_coords_wn[parent_id_p],
                            dtype=torch.float32,
                            device=device_plot,
                        )
                        e_child = torch.tensor(
                            final_e_coords_wn[child_id_p],
                            dtype=torch.float32,
                            device=device_plot,
                        )

                        e_path_pts = torch.stack(
                            [
                                e_parent + (e_child - e_parent) * t
                                for t in np.linspace(
                                    0, 1, num_path_interpolation_points_plot
                                )
                            ]
                        )
                        with torch.no_grad():
                            m_path_pts_tensor = trained_inn_phi_wn(
                                e_path_pts, reverse=True
                            )
                        ...
                        ax.plot(
                            ...
                            m_path_pts_np[:, 0],
                            linewidth=1.0
                            + path_curvature_val * 5000,  # Scale linewidth by curvature
                            alpha=0.8,
                            color=cmap_paths(norm(path_curvature_val)),
                            zorder=5,
                        )  # Plot paths under nodes for clarity if nodes are opaque

                sm_paths = plt.cm.ScalarMappable(cmap=cmap_paths, norm=norm)
                sm_paths.set_array([])
                cbar_paths = fig.colorbar(
                    sm_paths,
                    ax=ax,
                    label="Path Spatial Deviation",
                    shrink=0.5,
                    aspect=15,
                    pad=0.02,
                )
                ...
                print(
                    "Path curvature values are NaN or singular, skipping path coloring."
                )
    ...
    print(
        "Cannot plot WordNet manifold with paths as necessary data (m_coords, e_coords, or path_df) is missing."
    )
...
if (
    "trained_inn_phi_wn" not in locals()
    or "final_m_coords_wn_map" not in locals()
    or "df_analysis_wn" not in locals()
    or df_analysis_wn.empty
    or "df_paths_wn" not in locals()
    or df_paths_wn.empty
    or "hierarchy_pairs_wn" not in locals()
    or "final_e_coords_wn" not in locals()
):
    print(
        "Warning: One or more required WordNet variables not found. Using dummy data for plotting."
    )
    ...
    if "df_analysis_wn" not in locals() or df_analysis_wn.empty:
        ...
        for t in _tokens_dummy_wn:
            _dummy_analysis_data.append(
                {
                    "token": t,
                    "m_t": final_m_coords_wn_map[t][0],
                    "m_x": final_m_coords_wn_map[t][1],
                    "m_y": final_m_coords_wn_map[t][2],
                    "m_z": final_m_coords_wn_map[t][3],
    ...
    if "df_paths_wn" not in locals() or df_paths_wn.empty:
        ...
        for p, c in hierarchy_pairs_wn:
            if p is not None:
                _dummy_path_data.append(
                    {
                        "parent": p,
                        "child": c,
                        "curvature_metric": np.random.rand() * 0.0005,
                    }
                )
        df_paths_wn = pd.DataFrame(_dummy_path_data)
...
def plot_3d_manifold(
    ...
    if norm:  # Only add colorbar if normalization was possible
        cbar = fig.colorbar(
            scatter, label=actual_color_label, shrink=0.6, aspect=20, pad=0.1
        )
    ...
    if len(points) > 0:
        ...
        ax.auto_scale_xyz(
            [min_coords_plot[1], max_coords_plot[1]],
    ...
    for child_id, parent_id in hierarchy_pairs:
        if (
            parent_id is None
            or child_id not in m_coords_map
            or parent_id not in m_coords_map
        ):
            continue
        p_coords = m_coords_map[parent_id]
        c_coords = m_coords_map[child_id]
        ax.plot(
            [p_coords[1], c_coords[1]],
            [p_coords[2], c_coords[2]],
            [p_coords[0], c_coords[0]],
            color="gray",
            linestyle=":",
            alpha=0.4,
            linewidth=0.7,
...
if not df_analysis_wn.empty:
    ...
    plot_3d_manifold(
        final_m_coords_wn_map,
        hierarchy_pairs_wn,
        color_values=df_analysis_wn.set_index("token")["depth"]
        .reindex(final_m_coords_wn_map.keys())
        .values,
        color_label="WordNet Depth",
        cmap="cool",
        title="Learned WordNet Manifold ($m^*$) - Nodes Colored by Depth",
        tokens_to_label=set(tokens_wn),
    )  # Label all for WordNet if not too many

    # Plot 2: Color by Total Node Degree
    plot_3d_manifold(
        final_m_coords_wn_map,
        hierarchy_pairs_wn,
        color_values=df_analysis_wn.set_index("token")["total_degree"]
        .reindex(final_m_coords_wn_map.keys())
        .values,
        color_label="Total Node Degree",
        cmap="plasma",
    ...
    plot_3d_manifold(
        final_m_coords_wn_map,
        hierarchy_pairs_wn,
        color_values=df_analysis_wn.set_index("token")["g_tt"]
        .reindex(final_m_coords_wn_map.keys())
        .values,
        color_label="$g_{tt}$ (Time Component)",
        cmap="plasma",  # _r reverses coolwarm
    ...
    plot_3d_manifold(
        final_m_coords_wn_map,
        hierarchy_pairs_wn,
        color_values=df_analysis_wn.set_index("token")["avg_g_spatial_diag"]
        .reindex(final_m_coords_wn_map.keys())
        .values,
        color_label="Avg. Spatial Diagonal ($g_{ii}$)",
        cmap="viridis",
...
if "plot_m_embedding_paths_colored_by_curvature_labeled" not in locals():
    ...
    def plot_m_embedding_paths_colored_by_curvature_labeled(*args, **kwargs):
        print(
            "Warning: `plot_m_embedding_paths_colored_by_curvature_labeled` not defined. Skipping detailed path plot."
        )
        # You could call the simpler plot_3d_manifold here as a fallback
        if args:
            plot_3d_manifold(
                args[0], hierarchy_pairs_wn, title=kwargs.get("title", "Manifold Plot")
            )
...
if final_m_coords_wn_map and final_e_coords_wn and not df_paths_wn.empty:
    ...
    df_paths_wn_sorted_for_plot = df_paths_wn.sort_values(
        by="curvature_metric", ascending=False
    )
    paths_to_include_for_labeling = pd.concat(
        [df_paths_wn_sorted_for_plot.head(3), df_paths_wn_sorted_for_plot.tail(3)]
    )

    for _, row in paths_to_include_for_labeling.iterrows():
        if row["parent"] in final_m_coords_wn_map:
            tokens_for_path_plot_labels.add(row["parent"])
        if row["child"] in final_m_coords_wn_map:
            tokens_for_path_plot_labels.add(row["child"])
    ...
    if len(tokens_for_path_plot_labels) < 5 and not df_analysis_wn.empty:
        ...
        top_degree_nodes = (
            df_analysis_wn.sort_values(by="total_degree", ascending=False)["token"]
            .head(3)
            .tolist()
        )
        for node in top_degree_nodes:
            if node in final_m_coords_wn_map:
                tokens_for_path_plot_labels.add(node)
            if len(tokens_for_path_plot_labels) >= 7:
                break  # Limit total labels
    ...
    print(
        "Cannot plot WordNet manifold with paths colored by curvature as necessary data is missing."
    )
...
if (
    ...
    if "df_analysis_wn" not in locals() or df_analysis_wn.empty:
        ...
        for t in _tokens_dummy_wn:
            ...
            _g_M_dummy = (
                np.diag([-0.9, 0.9, 0.9, 0.9]) + (np.random.rand(4, 4) - 0.5) * 0.1
            )
            _dummy_analysis_data.append(
                {
                    "token": t,
        ...
        df_analysis_wn = pd.DataFrame(_dummy_analysis_data)
    if (
        "df_paths_wn" not in locals() or df_paths_wn.empty
    ):  # df_paths_wn should be recreated
        df_paths_wn = (
            pd.DataFrame()
        )  # Ensure it's an empty df if not properly generated
...
def analyze_local_geometry(m_coord_np, phi_network_model, eta_E):
    ...
    det_g_spatial = np.linalg.det(spatial_submatrix)
    spatial_volume_factor = (
        np.sqrt(np.abs(det_g_spatial))
        if (det_g_spatial > 0 or not np.isclose(det_g_spatial, 0))
        else 0.0
    )
    time_space_mixing = (
        np.sum(np.abs(g_M_np[0, 1:])) + np.sum(np.abs(g_M_np[1:, 0]))
    ) / 2
    return {
        "g_M": g_M_np,
        "det_g_M": det_g_M,
...
def calculate_m_path_length_and_deviation(
    m_path_points_np, phi_model_for_gM, eta_E_tensor_device, path_name=""
):  # Added path_name for debug
    """
    ...
    for i in range(len(m_path_points_np) - 1):
        ...
        geo_features_mid = analyze_local_geometry(
            m_midpoint_segment, phi_model_for_gM, eta_E_tensor_device
        )
        ...
        if (
            ds_sq_segment_m < -1e-9
        ):  # Timelike segment on M (add small tolerance for strict negativity)
...
if final_e_coords_wn and final_m_coords_wn_map:
    ...
    for parent_id, child_id in hierarchy_pairs_wn:
        ...
        e_child = torch.tensor(
            final_e_coords_wn[child_id], dtype=torch.float32, device=device_for_analysis
        )

        e_path_pts_tensor = torch.stack(
            [
                e_parent + (e_child - e_parent) * t
        ...
        path_len_m, spatial_dev = calculate_m_path_length_and_deviation(
            m_path_pts_np, trained_inn_phi_wn, eta_E_torch_device
        )

        degree_parent = (
            df_analysis_wn.set_index("token").loc[parent_id, "total_degree"]
            if parent_id in df_analysis_wn.set_index("token").index
            else 0
        )
        degree_child = (
            df_analysis_wn.set_index("token").loc[child_id, "total_degree"]
            if child_id in df_analysis_wn.set_index("token").index
            else 0
        )
        ...
        path_analysis_data_wn.append(
            {
                ...
                "parent_g_tt": df_analysis_wn.set_index("token").loc[parent_id, "g_tt"]
                if parent_id in df_analysis_wn.set_index("token").index
                else np.nan,
                "child_g_tt": df_analysis_wn.set_index("token").loc[child_id, "g_tt"]
                if child_id in df_analysis_wn.set_index("token").index
                else np.nan,
            }
        )
    ...
    if not df_paths_analysis_wn.empty:
        ...
        df_paths_wn = df_paths_analysis_wn.rename(
            columns={"spatial_deviation_m": "curvature_metric"}
        )
    ...
    print(
        "final_e_coords_wn or final_m_coords_wn_map is empty. Skipping path analysis."
    )
...
def plot_m_embedding_paths_len_curve(
    ...
    if (
        node_color_feature_df is not None
        and node_color_by in node_color_feature_df.columns
    ):
        # Align feature values with the order of points_plot
        node_feature_values_series = node_color_feature_df.set_index("token")[
            node_color_by
        ].reindex(token_ids_plot)
    ...
    # Path plotting
    if (
        not path_data_df.empty
        and "curvature_metric" in path_data_df.columns
        and "path_length_m" in path_data_df.columns
    ):
        curvatures = path_data_df["curvature_metric"].values
        lengths = path_data_df["path_length_m"].values

        # Normalize curvature for color
        norm_curvature = plt.Normalize(
            vmin=np.nanmin(curvatures) if not np.all(np.isnan(curvatures)) else 0,
        ...
        if min_len == max_len:
            max_len = min_len + 1  # Avoid division by zero for norm
        ...
        for idx, row in path_data_df.iterrows():
            ...
            if parent_id in final_e_coords_wn and child_id in final_e_coords_wn:
                ...
                e_c = torch.tensor(
                    final_e_coords_wn[child_id], dtype=torch.float32, device=path_device
                )

                e_path = torch.stack(
                    [
                        e_p + (e_c - e_p) * t
                        for t in np.linspace(0, 1, num_path_interp_plot)
                    ]
                )
                with torch.no_grad():
                ...
                linewidth = (
                    0.5 + 2.5 * ((path_len - min_len) / (max_len - min_len + 1e-9))
                    if max_len > min_len
                    else 1.5
                )
        ...
        fig.colorbar(
            sm_curvature,
            ax=ax,
            label="Path Spatial Deviation (Curvature)",
            shrink=0.5,
            aspect=15,
            pad=0.02,
        )
...
if (
    final_m_coords_wn_map
    and final_e_coords_wn
    and not df_paths_wn.empty
    and not df_analysis_wn.empty
):
    # Select tokens to label for clarity
    tokens_to_label_plot_wn = set()
    if not df_paths_wn.empty:
        df_paths_wn_sorted = df_paths_wn.sort_values(
            by="curvature_metric", ascending=False
        )
        paths_to_include = pd.concat(
            [df_paths_wn_sorted.head(2), df_paths_wn_sorted.tail(2)]
        )  # Most & least curved
        for _, row in paths_to_include.iterrows():
            if row["parent"] in final_m_coords_wn_map:
                tokens_to_label_plot_wn.add(row["parent"])
            if row["child"] in final_m_coords_wn_map:
                tokens_to_label_plot_wn.add(row["child"])
    if len(tokens_to_label_plot_wn) < 7:  # Ensure some labels if few paths chosen
        tokens_to_label_plot_wn.update(
            list(
                df_analysis_wn.sort_values(by="total_degree", ascending=False)[
                    "token"
                ].head(max(0, 7 - len(tokens_to_label_plot_wn)))
            )
        )
    if not tokens_to_label_plot_wn and tokens_wn:
        tokens_to_label_plot_wn.add(tokens_wn[0])  # At least one label
...
if not df_paths_analysis_wn.empty:
    ...
    sns.scatterplot(
        data=df_paths_analysis_wn, x="avg_degree_of_pair", y="path_length_m"
    )
    sns.regplot(
        data=df_paths_analysis_wn,
        x="avg_degree_of_pair",
        y="path_length_m",
        scatter=False,
        color="green",
    ...
    plt.xlabel("Average Total Degree")
    plt.ylabel("Path Length on M")
    plt.grid(True, linestyle=":", alpha=0.7)

    plt.subplot(1, 3, 2)
    sns.scatterplot(
        data=df_paths_analysis_wn, x="spatial_deviation_m", y="path_length_m"
    )
    sns.regplot(
        data=df_paths_analysis_wn,
        x="spatial_deviation_m",
        y="path_length_m",
        scatter=False,
        color="purple",
    )
    plt.title("M-Path Length vs. Spatial Deviation")
    plt.xlabel("Path Spatial Deviation (Curvature)")
    plt.ylabel("Path Length on M")
    plt.grid(True, linestyle=":", alpha=0.7)

    plt.subplot(1, 3, 3)
    # Example: Path length vs. average g_tt of parent and child
    df_paths_analysis_wn["avg_g_tt_of_pair"] = (
        df_paths_analysis_wn["parent_g_tt"] + df_paths_analysis_wn["child_g_tt"]
    ) / 2.0
    ...
    plt.xlabel("Average $g_{tt}$ of Parent and Child")
    plt.ylabel("Path Length on M")
    plt.grid(True, linestyle=":", alpha=0.7)
    ...
    if len(df_paths_analysis_wn) > 1:
        ...
        print(
            f"  vs. Avg Degree: {df_paths_analysis_wn['path_length_m'].corr(df_paths_analysis_wn['avg_degree_of_pair']):.4f}"
        )
        print(
            f"  vs. Spatial Deviation: {df_paths_analysis_wn['path_length_m'].corr(df_paths_analysis_wn['spatial_deviation_m']):.4f}"
        )
        print(
            f"  vs. Avg g_tt of Pair: {df_paths_analysis_wn['path_length_m'].corr(df_paths_analysis_wn['avg_g_tt_of_pair']):.4f}"
        )
...
def train_inn_phi_embedding_with_metric_reg_v5(  # Renamed for clarity
    ...
    initial_m_coords_map = {
        token_id: np.array(
            [0.0] + np.random.uniform(-0.1, 0.1, inn_phi_model.input_dim - 1).tolist(),
            dtype=np.float32,
        )
        for token_id in tokens
    }
    ...
    print(
        f"g_tt_mse_w={g_tt_mse_weight}, g_tt_neg_w={g_tt_negativity_penalty_weight}"
    )  # Updated name
    print(
        f"g_sp_diag_mse_w={g_spatial_diag_mse_weight}, g_sp_pos_w={g_spatial_positivity_penalty_weight}"
    )
    print(
        f"g_off_diag_mse_w={g_off_diag_mse_weight}, clip={gradient_clip_value}, reg_midpoints={regularize_path_midpoints}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for child_id, parent_id in training_pairs_ids:
            ...
            m_points_for_gM_reg = [
                m_coords_child_tensor_input,
                m_coords_parent_tensor_input,
            ]
            ...
            if regularize_path_midpoints:
                with torch.no_grad():  # Inverse pass doesn't need grad for this purpose
                    ...
                    for k in range(1, num_midpoints_to_reg + 1):
                        ...
                        e_mid = (
                            e_coords_parent
                            + (e_coords_child - e_coords_parent) * t_interp
                        )
                        m_mid = inn_phi_model(e_mid.unsqueeze(0), reverse=True).squeeze(
                            0
                        )
            ...
            for m_point_tensor in m_points_for_gM_reg:
                g_M = get_g_M_for_loss(
                    m_point_tensor, inn_phi_model, eta_E_torch_for_loss
                )

                # g_tt regularization
                g_tt_val = g_M[0, 0]
                loss_g_tt_m_pt = mse_loss_fn(
                    g_tt_val, torch.tensor(g_tt_target, device=device)
                )
                ...
                loss_g_tt_s_pt = torch.relu(
                    g_tt_val - torch.tensor(g_tt_target, device=device)
                )
                step_g_tt_loss_sum += (
                    g_tt_mse_weight * loss_g_tt_m_pt
                    + g_tt_negativity_penalty_weight * loss_g_tt_s_pt
                )

                # Spatial diagonal regularization
                spatial_diagonals = torch.diag(g_M[1:, 1:])
                target_spatial_diag_tensor = torch.full(
                    (inn_phi_model.input_dim - 1,), g_spatial_diag_target, device=device
                )
                loss_g_sp_diag_m_pt = mse_loss_fn(
                    spatial_diagonals, target_spatial_diag_tensor
                )
                loss_g_sp_pos_pt = torch.sum(
                    torch.relu(-spatial_diagonals)
                )  # Penalize if any g_ii < 0
                step_g_sp_diag_loss_sum += (
                    g_spatial_diag_mse_weight * loss_g_sp_diag_m_pt
                )
                step_g_sp_pos_loss_sum += (
                    g_spatial_positivity_penalty_weight * loss_g_sp_pos_pt
                )

                # Off-diagonal regularization
                g_M_offdiag_pt = g_M.clone()
                g_M_offdiag_pt.diagonal().fill_(0)
                loss_g_off_diag_m_pt = mse_loss_fn(
                    g_M_offdiag_pt, torch.zeros_like(g_M_offdiag_pt)
                )
            ...
            current_g_spatial_diag_loss_val = (
                step_g_sp_diag_loss_sum / num_reg_points
            ).item()
            current_g_spatial_pos_loss_val = (
                step_g_sp_pos_loss_sum / num_reg_points
            ).item()
            current_g_off_diag_loss_val = (
                step_g_off_diag_loss_sum / num_reg_points
            ).item()
        ...
        avg_interval = np.mean(epoch_losses_agg["interval"])
        avg_g_tt = np.mean(epoch_losses_agg["g_tt"])
        avg_g_spatial_diag = np.mean(
            epoch_losses_agg["g_spatial_diag"]
        )  # MSE part for spatial diag
        avg_g_spatial_pos = np.mean(
            epoch_losses_agg["g_spatial_pos"]
        )  # Positivity penalty for spatial diag
        avg_g_off_diag = np.mean(epoch_losses_agg["g_off_diag"])
        ...
        if epoch % log_interval == 0 or epoch == num_epochs - 1:
            ...
            if len(tokens) > 0:
                with torch.no_grad():
                    ...
                    sample_m_tensor_log = initial_m_coords_tensors_map[
                        sample_token_id_log
                    ]
                    g_M_sample_log = get_g_M_for_loss(
                        sample_m_tensor_log, inn_phi_model, eta_E_torch_for_loss
                    )
                    print(
                        f"  Sample g_M for '{sample_token_id_log}' (g_tt: {g_M_sample_log[0, 0].item():.3f}, "
...
if "tokens_inn_reg_v4" not in locals():  # Basic check
    tokens_inn_reg_v4 = ["a", "b", "c"]
    hierarchy_pairs_inn_reg_v4 = [("b", "a"), ("c", "b")]
...
initial_m_coords_reg_v5, final_e_coords_reg_v5, trained_inn_phi_reg_v5 = (
    train_inn_phi_embedding_with_metric_reg_v5(
        tokens_inn_reg_v4,
        hierarchy_pairs_inn_reg_v4,  # Use existing token/pair lists
...
if (
    final_e_coords_reg_v5
    and "universe" in final_e_coords_reg_v5
    and "analyze_local_geometry" in locals()
):
    ...
    e_final_univ_tensor_v5 = torch.tensor(e_final_univ_np_v5, dtype=torch.float32).to(
        _device
    )
    with torch.no_grad():
        m_star_univ_tensor_v5 = trained_inn_phi_reg_v5(
            e_final_univ_tensor_v5.unsqueeze(0), reverse=True
        ).squeeze(0)
    m_star_univ_np_v5 = m_star_univ_tensor_v5.cpu().numpy()

    univ_geo_features_v5 = analyze_local_geometry(
        m_star_univ_np_v5, trained_inn_phi_reg_v5, eta_E_torch_for_loss.to(_device)
    )
...
plt.colorbar(label="Gravity Potential (−log density)")
