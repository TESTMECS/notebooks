...
results = [
    random.choices(targets, weights=probabilities, k=1)[0] for _ in range(num_runs)
]
...
nx.draw_networkx_labels(
    causal_tree,
    pos=layered_pos,
    labels={n: n.split("_")[0] for n in causal_tree.nodes()},
    font_size=8,
)
...
nx.draw_networkx_labels(
    gravity_tree,
    pos=layered_pos,
    labels={n: n.split("_")[0] for n in gravity_tree.nodes()},
    font_size=8,
)
...
def extract_geodesic(graph, start_node):
            ...
            for leaf in leaves:
                try:
                    ...
                    path = nx.shortest_path(
                        temp_graph, start_node, leaf, weight="weight"
                    )
    ...
    for node in topo_order:
        for succ in graph.successors(node):
            ...
            new_weight = best_weight.get(node, float("-inf")) + np.log(
                edge_weight + 1e-9
            )  # log-prob path
...
for u, v in gravity_tree.edges():
    if (
        u in geodesic_path
        and v in geodesic_path
        and geodesic_path.index(v) == geodesic_path.index(u) + 1
    ):
...
node_colors = [
    "gold" if n in geodesic_path else "lightgray" for n in gravity_tree.nodes()
]
...
for u, v in attention_graph.edges():
    if (
        u in geodesic_attention_path
        and v in geodesic_attention_path
        and geodesic_attention_path.index(v) == geodesic_attention_path.index(u) + 1
    ):
        edge_colors.append("crimson")
    else:
        edge_colors.append("gray")
...
node_colors = [
    "gold" if n in geodesic_attention_path else "lightgray"
    for n in attention_graph.nodes()
]
node_sizes = [
    1200 if n in geodesic_attention_path else 300 for n in attention_graph.nodes()
]
...
def project_until_convergence_stable(
    ...
    delta_spatial_norm_all_pairs = np.linalg.norm(
        spatial_desc_all_pairs - spatial_anc_all_pairs, axis=1
    )
    ...
    with Progress(
        ...
        stable_task = progress.add_task("[blue]Stable convergence...", total=max_passes)
        ...
        for n_pass in range(max_passes):
            ...
            if (
                np.isnan(delta_spatial_norm_all_pairs).any()
                or np.isinf(delta_spatial_norm_all_pairs).any()
                or np.isnan(delta_t_all_pairs).any()
                or np.isinf(delta_t_all_pairs).any()
            ):
                progress.stop()
                console.print(
                    Panel(
            ...
            is_violation = (
                delta_t_all_pairs <= delta_spatial_norm_all_pairs + eps1_margin
            )
            num_violations = np.sum(is_violation)

            if num_violations == 0:
                progress.update(
                    stable_task, description="[green]✅ Converged (0 violations)!"
                )
            ...
            if (
                passes_since_improvement >= stagnation_patience
                and current_step_factor > min_step_factor
            ):
                current_step_factor = max(
                    min_step_factor, current_step_factor * step_decay_rate
                )
                passes_since_improvement = 0
            ...
            delta_to_satisfy_condition = (
                delta_spatial_norm_viol + eps1_margin
            ) - delta_t_viol
            delta_to_satisfy_condition = np.maximum(
                0, delta_to_satisfy_condition
            )  # Ensure non-negative

            # Apply safety cap
            delta_to_satisfy_condition = np.minimum(
                delta_to_satisfy_condition, max_delta_cap
            )
            ...
            if max_val_in_applied_total_delta < convergence_tolerance:
                progress.update(
                    stable_task,
                    description="[green]✅ Converged (minimal adjustments)!",
                )
                return n_pass
            ...
            anc_time_updates = (
                -applied_total_delta * eps2_distrib
            )  # Negative sign for decreasing ancestor time

            if (
                np.isnan(desc_time_updates).any()
                or np.isinf(desc_time_updates).any()
                or np.isnan(anc_time_updates).any()
                or np.isinf(anc_time_updates).any()
            ):
                progress.stop()
                return max_passes + 5

            np.add.at(
                time_vec, viol_desc_indices, desc_time_updates.astype(time_vec.dtype)
            )
            np.add.at(
                time_vec, viol_anc_indices, anc_time_updates.astype(time_vec.dtype)
            )
            ...
            if (n_pass + 1) % verbose_interval == 0:
                progress.update(
                    stable_task,
                    description=f"[yellow]Pass {n_pass + 1}: {num_violations} violations (SF: {current_step_factor:.4f})",
                )
...
if len(attention_pairs) > 0:
    ...
    console.print(
        f"[cyan]Found {len(desc_anc_pairs)} attention-based causal pairs[/cyan]"
    )
    ...
    if convergence_result < 1000:
        console.print(
            f"[green]✅ Stable convergence achieved in {convergence_result} passes![/green]"
        )
    else:
        console.print(
            f"[red]⚠️ Convergence issue: result code {convergence_result}[/red]"
        )
else:
    console.print(
        "[yellow]No attention pairs found above threshold, using simple algorithm...[/yellow]"
    )
    # Fallback to simple algorithm if no pairs found

console.print(
    f"[green]Final time coordinate range: [{np.min(t_coords):.3f}, {np.max(t_coords):.3f}][/green]"
)
        ...
        for j in range(seq_len):
            if A[i, j] > THRESHOLD:
                ...
                if (
                    ...
                    or np.isinf(t[i])
                    or np.isinf(t[j])
                    or np.any(np.isnan(x[i]))
                    or np.any(np.isnan(x[j]))
                    or np.any(np.isinf(x[i]))
                    or np.any(np.isinf(x[j]))
                ):
                    continue
...
for i in range(len(x_spatial)):
    ax1.scatter(
        [x_spatial[i, 0]], [x_spatial[i, 1]], [t_coords[i]], c="gold", s=100, alpha=0.8
    )
...
def semantic_projection(token_embedding, use_pca=True):
    ...
    if use_pca:
        ...
        pca_weights = np.random.randn(
            token_embedding.shape[-1], 3
        ...
        t = (
            np.mean(token_embedding[:100])
            if len(token_embedding) > 100
            else np.mean(token_embedding)
        )
        x = (
            np.mean(token_embedding[100:200])
            if len(token_embedding) > 200
            else np.std(token_embedding)
        )
        y = (
            np.mean(token_embedding[200:300])
            if len(token_embedding) > 300
            else np.var(token_embedding)
        )
...
def multi_geodesic_beam_step(beams, model, tokenizer, beam_width=5, alpha=0.8):
    ...
    for beam in beams:
        ...
        for token_id, prob in zip(topk.indices, topk.values):
            ...
            token_embedding = (
                model.get_input_embeddings()(token_id.unsqueeze(0))
                .detach()
                .numpy()
                .squeeze()
            )
            ...
            new_momentum = (
                new_coord - beam.coords[-1]
                if len(beam.coords) > 0
...
# Generate coordinates for all input tokens
init_coords = []
for token_id in input_ids:
    embedding = (
        model.get_input_embeddings()(token_id.unsqueeze(0)).detach().numpy().squeeze()
    )
    coord = semantic_projection(embedding)
    init_coords.append(coord)

console.print(f"[cyan]Initial prompt:[/cyan] '{prompt}'")
console.print(f"[cyan]Initial coordinates:[/cyan] {len(init_coords)} tokens")
...
console.print(
    f"[yellow]Running {num_steps} steps of geodesic beam search with width {beam_width}...[/yellow]"
)

for step in range(num_steps):
    beams = multi_geodesic_beam_step(
        beams, model, tokenizer, beam_width=beam_width, alpha=0.8
    )
    if step % 3 == 0:
...
for i, beam in enumerate(beams[:3]):
    ...
    console.print(f"[yellow]  Token breakdown:[/yellow]")
    tokens_decoded = tokenizer.convert_ids_to_tokens(beam.tokens)
    for j, (token_id, token_str, coord) in enumerate(zip(beam.tokens, tokens_decoded, beam.coords)):
        token_decoded = tokenizer.decode([token_id])
        console.print(f"    {j}: '{token_str}' → '{token_decoded}' → [{coord[0]:.3f}, {coord[1]:.3f}, {coord[2]:.3f}]")
    console.print()
...
console.print(
    "[green]✅ All syntax errors fixed! Geodesic beam search demo complete.[/green]"
)
