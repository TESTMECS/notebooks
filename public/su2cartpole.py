...
def _(mo):
    mo.md(
        r"""
    This notebook showcases the implementation of a Q-Learning agent that leverages SU(2) symmetry for the CartPole environment. It explores how SU(2) transformations, represented by learnable gates, can be integrated into the agent's action space and state representation.

    The core idea is to represent the agent's state as a spinor (a 2-component complex vector) and to use SU(2) transformations as actions. This approach is inspired by quantum mechanics, where SU(2) plays a fundamental role in describing spin.
    """
    )
    return
...
def _():
    ...
    return F, deque, gym, mo, nn, norm, np, random, torch
    ...
    class SU2Attention(nn.Module):
        ...
        def forward(self, spinors):
            ...
            attn_logits = torch.einsum(
                "bijhd,bijhd->bijh", spinor_conj, spinor_base
            )  # (B, T, T, H)
            ...
            out = torch.einsum(
                "bijh,bjhd->bihd", attn_weights_complex, spinors
            )  # (B, T, H, 2)
            return out
    mo.show_code()
    return
...
def _(mo, nn, torch):
    class SU2Gate(nn.Module):
        ...
        def forward(self, spinors):
            ...
            G = torch.cos(theta / 2) * I - 1j * torch.sin(theta / 2) * (
                nx * σx + ny * σy + nz * σz
            )
            return spinors @ G
    mo.show_code()
...
@app.cell
def _(F, SU2Gate, deque, mo, nn, random, torch):
    class QLearningAgent(nn.Module):
        ...
        def replay(self):
            ...
            current_q_values = self.obs_encoder(obs_batch).gather(
...
def _(QLearningAgent, gym, mo, torch):
    ...
    for episode in mo.status.progress_bar(range(EPOCHS)):  # More episodes for learning
        ...
        for t in range(MAX_STEPS):  # CartPole max steps
