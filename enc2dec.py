import math
import random
import heapq # For priority queue if needed, or just managing bests
import numpy as np
...
def interaction_step(space, max_merges=3):
    ...
    for energy, e1, e2 in merge_candidates[:max_merges]:
        if e1.id in space.events and e2.id in space.events:
            # Create merged event
            merged_solution = [(e1.solution[0] + e2.solution[0])/2, 
            ...
            # Add merged event
            merged_event = space.add_event(merged_solution, merged_fitness, 
                                         merged_t, merged_x_emb, merged_y_emb)
    ...
    if merged_count > 0:
        console.print(f"[dim cyan]⚛️  Physics merged {merged_count} low-energy event pairs[/dim cyan]")
...
OBJECTIVE_FUNCTION = rastrigin_function # or sphere_function
PROBLEM_BOUNDS = [(-5.12, 5.12), (-5.12, 5.12)] # For Rastrigin
...
class Event:
    ...
    def __repr__(self):
        return f"E({self.id}, S=({self.solution[0]:.2f},{self.solution[1]:.2f}), F={self.fitness:.2f}, t={self.t:.2f}, emb=({self.x_emb:.2f},{self.y_emb:.2f}))"
...
class MinkowskiSpace:
    ...
    def add_event(self, solution, fitness, t_initial, x_emb_initial, y_emb_initial):
        ...
        event = Event(event_id, solution, fitness, t_initial, x_emb_initial, y_emb_initial)
    ...
    def get_causal_parents(self, event, max_count=10):
        ...
        for parent_id, parent_event in self.events.items():
            if parent_id != event.id and self.is_in_future_light_cone(parent_event, event):
                ...
                if ds2 < 0:  # Timelike
                    ...
                    score = fitness_improvement / (energy + 1e-6)  # Higher score = better causal connection
...
def project_until_convergence(
    ...
    for n_pass in range(1, max_passes + 1):
        ...
        if n_pass <= 5 or n_pass % 1000 == 0:
            console.print(f"[yellow]Pass {n_pass}: {violation.sum()} violations[/yellow]")
...
def project_and_update_embedding(new_event, space, potential_parents, iteration):
    ...
    # This logic is key and highly simplified here
    best_parent = None
    # Simplistic: pick the best fitness parent from recent history
    # A more complex rule would consider improvement, similarity, etc.
    sorted_parents = sorted(potential_parents, key=lambda p: p.fitness)
    if sorted_parents:
        best_parent = sorted_parents[0]

    if not best_parent: return

    # --- "Causality" Logic ---
    is_improvement = new_event.fitness < best_parent.fitness
    is_significant_improvement = new_event.fitness < best_parent.fitness * 0.9 # e.g. 10% better

    # Rule 1: Significant Improvement (Exploitation) -> Pull into near-null geodesic
    if is_significant_improvement:
        #print(f"  Significant improvement over parent {best_parent.id}. Pulling closer.")
    ...
    new_event.t = max(new_event.t, float(iteration) -0.5) # Allow some pull-back but not too much
...
class Agent:
    ...
    def move(self, global_best_event, space, iteration, objective_function, bounds):
        ...
        current_event = space.get_event(self.associated_event_id) if self.associated_event_id else None
        ...
        accept_move = fitness_improvement > 0 or (fitness_improvement > -0.1 and physics_penalty < 2.0)
        ...
        if accept_move:
            ...
            self.solution[0] += random.uniform(-0.01, 0.01) * (bounds[0][1] - bounds[0][0])
            self.solution[1] += random.uniform(-0.01, 0.01) * (bounds[1][1] - bounds[1][0])
...
class Runnable:
    ...
    def run(self):
        ...
        for i, agent in enumerate(agents):
            event = self._initial_embed_solution(agent.solution, agent.fitness, 0, space)
            ...
            for iteration in range(1, self.num_iterations + 1):
                    ...
                    for i, e1 in enumerate(events):
                        for j, e2 in enumerate(events[i+1:], i+1):
                            if e2.t > e1.t:  # Ensure time ordering
                                ...
                                improvement_label = 1.0 if e2.fitness < e1.fitness else 0.0
                                causal_training_data.append((features, improvement_label))
                
                for agent in agents:
                    if space.global_best_event:
                        agent.move(space.global_best_event, space, iteration, self.objective_function, self.bounds)

                    new_event = self._initial_embed_solution(agent.solution, agent.fitness, iteration, space)
                    agent.associated_event_id = new_event.id

                    potential_parents = space.get_recent_events(count=self.num_agents * 2)
                    potential_parents = [p for p in potential_parents if p.id != new_event.id]

                    # Prepare data for project_until_convergence
                    if potential_parents:
                        all_events = [new_event] + potential_parents
                        pairs = np.array([[0, i+1] for i in range(len(potential_parents))])
                        
                        spatial = np.array([[e.x_emb, e.y_emb] for e in all_events])
                        time_vec = np.array([e.t for e in all_events])
                        
                        passes_taken = project_until_convergence(pairs, spatial, time_vec)
                ...
                if iteration % 20 == 0:
                    ...
                    if len(causal_training_data) > 50:
                        self._train_causal_predictor(causal_training_data[-100:])  # Use recent data

                if iteration % (self.num_iterations // 10 or 1) == 0:
                    energy = physics_energy_history[-1] if physics_energy_history else 0
                    progress.update(main_task, description=f"[cyan]Iter {iteration}: Best={space.global_best_event.fitness:.6f}, Energy={energy:.2f}")

        console.print(f"\n[bold green]Physics-Enhanced Optimization Complete![/bold green]")
        console.print(f"[green]Best Solution:[/green] {space.global_best_event.solution}")
        ...
        console.print(f"[green]Causal Training Samples:[/green] {len(causal_training_data)}")
    ...
    def _create_agent(self, agent_id):
        ...
        agent.solution = [random.uniform(self.bounds[0][0], self.bounds[0][1]),
...
best_event_found, final_space = Runnable(OBJECTIVE_FUNCTION, PROBLEM_BOUNDS).run()
...
def visualize_embedding_trajectory(events):
    ...
    colors = plt.cm.viridis((np.array(fitness) - min(fitness)) / (max(fitness) - min(fitness)))
...
def visualize_causal_edges(events, space):
    ...
    nx.draw(G, pos, node_size=30, edge_color='gray', width=edge_widths, node_color='blue', alpha=0.6)
...
def visualize_bottlenecks(events, space):
    ...
    node_colors = ['red' if node in dict(top_bottlenecks) else 'blue' for node in G.nodes()]
    plt.figure(figsize=(10, 8))
...
def visualize_clusters(events):
    ...
    plt.figure(figsize=(8,6))
    scatter = plt.scatter(embeddings[:,0], embeddings[:,1], c=labels, cmap='tab10', s=50)
...
def plot_physics_energy_map(events, space):
        ...
        plt.figure(figsize=(8,6))
        sc = plt.scatter(dt_list, np.sqrt(np.array(dx_list)**2 + np.array(dy_list)**2), 
                         c=energy_list, cmap='plasma', alpha=0.7)
...
def animate_agent_trails(agent_traces, bounds=[(-1,1), (-1,1)]):
        ...
        ani = FuncAnimation(fig, update, frames=max(len(t) for t in agent_traces), interval=200)
        plt.close()
        return ani
...
if __name__ == "__main__":
    ...
    events = list(space3.events.values())
    ...
    for i, e1 in enumerate(events):
        for e2 in events[i+1:]:
            if space3.is_in_future_light_cone(e1, e2):
    ...
    analysis_table.add_row("Avg Energy/Connection", f"{total_energy/max(causal_connections,1):.4f}")
    analysis_table.add_row("Solution Quality", f"{best3.fitness:.6f}")
    ...
    animate_agent_trails(best3.agent_traces, bounds=[(-1,1), (-1,1)])
