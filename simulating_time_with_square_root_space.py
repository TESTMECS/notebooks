...
def draw_tree(G):
    ...
    plt.title(
        "Tree Structure for Simulating Binary Addition via Blocks"
    )
    plt.tight_layout()
    plt.show()
...
class TreeNode:
    def __init__(
        self, label, left=None, right=None, is_leaf=False, value=None
    ):
        self.label = label
        self.left = left
        self.right = right
...
def evaluate_tree(node, depth=0):
    ...
    print(
        "  " * depth
        + f"{node.label} = {left_val} + {right_val} = {node_value}"
    )
    return node_value
...
class Event:
    ...
    def causally_connects(self, other, epsilon=1e-5):
        return (
            self.interval_to(other) <= -epsilon and self.t < other.t
        )
...
def plot_with_null_geodesics(events, causal_G, null_edges):
    ...
    nx.draw_networkx_edges(
        causal_G, pos, edge_color="black", arrowsize=20
    )
    nx.draw_networkx_nodes(
        causal_G, pos, node_color="lightblue", node_size=900
    )
    nx.draw_networkx_labels(
        causal_G, pos, labels=labels, font_size=10
    )
    ...
    plt.title(
        "Causal Graph with Null Geodesics (Reversible Simulation Paths)"
    )
    plt.legend()
    plt.axis("equal")
    plt.show()
...
class ReversibleTuringNet(InteractionNet):
    ...
    def reversible_head_step(self, head_id, direction=1):
        ...
        self.time += 1
        new_label_head = (
            label_neighbor.split("@")[0] + f"@{self.time}"
        )
        new_label_neighbor = (
            label_head.split("@")[0] + f"@{self.time}"
        )
...
class StatefulTuringNet(ReversibleTuringNet):
    ...
    def reversible_write_and_move(
        self, head_id, direction, new_symbol, next_state
    ):
        neighbors = (
            list(self.graph.successors(head_id))
            if direction == 1
        ...
        current_state, _ = (
            meta.split("@")
            if "@" in meta
            else (self.head_state, self.time)
        )
        ...
        new_head_label = (
            f"{neighbor_label.split('@')[0]}|{next_state}@{self.time}"
        )
...
state_net.reversible_write_and_move(
...
class BinaryAdditionMinkowskiSim:
    ...
    def visualize_minkowski(self):
        ...
        ax.set_title(
            "Minkowski Spacetime Embedding of Binary Addition"
        )
...
simulator = BinaryAdditionMinkowskiSim(
    "1011", "0110"
)  # 11 + 6 = 17 → 10001
...
def plot_causal_cones_with_arrows(events, null_threshold=1e-3):
    ...
    ax.set_zlabel("Time (t)")
    ax.set_title(
        "Causal Cones and Null Geodesics for Binary Addition Events"
    )
    plt.tight_layout()
    plt.show()
...
def visualize_metric_space(events, df_costs):
    ...
    for _, row in df_costs.iterrows():
        ...
        ax.plot(
            ...
            color=color,
            linestyle="--"
            if row["relation"] == "reversible-null"
            else "-",
            linewidth=2,
            alpha=0.8,
        )
...
# %% =================================
def visualize_metric_space_with_entropy(events, df_costs):
    """
    Visualize the metric space with irreversible steps highlighted
    In this case the all the steps are irreversible. and the spacelike steps are ones that
    are the ones that are not causally connected(i.e. not in the same causal cone).
    """
    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111, projection="3d")
...
visualize_metric_space_with_entropy(entropic_sim.events, df_entropy)
# %% ================================================
def compute_global_loss(events, alpha=1.0, beta=2.0, gamma=1.0):
    """
    Compute the global loss function with 3 components: irreversible, causally disconnected, false independence
    """
    irreversible_cost = 0
    disconnected_penalty = 0
    false_independence_penalty = 0
    event_ids = {e[0]: e for e in events}

    for e1, e2 in combinations(events, 2):
        if e1[3] >= e2[3]:
            continue  # only consider forward-time pairs
        s2 = minkowski_interval(e1, e2)
        dt = e2[3] - e1[3]

        if abs(s2) < 1e-3:
            continue  # reversible, skip
        elif s2 < -1e-3:
            irreversible_cost += alpha * np.sqrt(-s2)
        elif s2 > 0 and dt <= 2:
            false_independence_penalty += (
                gamma * s2
            )  # penalize spacelike close steps

    # Causal coherence penalty: no links forward from i
    for i, e1 in enumerate(events[:-1]):
        successors = [
            e2
            for e2 in events[i + 1 :]
            if minkowski_interval(e1, e2) < 0 and e2[3] > e1[3]
        ]
        if not successors:
            disconnected_penalty += beta
    ...
    return {
        ...
        "Irreversible Cost": irreversible_cost,
        "Disconnected Penalty": disconnected_penalty,
        "False Independence Penalty": false_independence_penalty,
    }


# %% ================================================
loss = compute_global_loss(entropic_sim.events)
print(loss)