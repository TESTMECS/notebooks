...
class NotebookAnalyzer:
    ...
    def analyze_notebook(self, notebook_path: Path) -> Dict:
        ...
        for cell in cells:
            ...
            if cell_type == "code":
                ...
                # Extract imports
                for line in source:
                    if isinstance(line, str) and (line.strip().startswith("import ") or line.strip().startswith("from ")):
        ...
        analysis["key_concepts"] = self._extract_key_concepts(analysis["title"], analysis["summary"])
        analysis["topics"] = self._categorize_notebook(analysis["title"], analysis["summary"])
    ...
    def _extract_key_concepts(self, title: str, summary: str) -> List[str]:
        ...
        concept_patterns = {
            ...
            "mathematics": ["algorithm", "optimization", "matrix", "vector", "function"],
...
class StickyNotesManager:
    ...
    def add_note(self, notebook: str, note: str, category: str = "general"):
        ...
        note_data = {
            "id": hashlib.md5(f"{notebook}{note}{datetime.now()}".encode()).hexdigest()[:8],
            "content": note,
            "category": category,
    ...
    def delete_note(self, notebook: str, note_id: str):
        """Delete a note"""
        if notebook in self.notes:
            self.notes[notebook] = [n for n in self.notes[notebook] if n["id"] != note_id]
            self._save_notes()
            return True
        return False
...
class QuoteGenerator:
    ...
    def get_quote(self, themes: Optional[List[str]] = None, seed: Optional[int] = None) -> str:
...
class NotebooksCLI:
    ...
    def print_notebook_summary(self, analysis: Dict):
        ...
        info_table.add_row("📅 Modified:", analysis['modified'].strftime('%Y-%m-%d %H:%M:%S'))
        info_table.add_row("📏 Size:", f"{analysis['size']:,} bytes")
        info_table.add_row("🔢 Cells:", f"{analysis['cells']} total ({analysis['code_cells']} code, {analysis['markdown_cells']} markdown)")
        ...
        # Imports panel
        if analysis['imports']:
            imports_text = "\n".join([f"• {imp}" for imp in sorted(list(analysis['imports'])[:5])])
            imports_panel = Panel(
                imports_text,
                title="📦 Key Imports",
                title_align="left",
        ...
        # Images indicator
        if analysis['images']:
            console.print(f"🖼️  Contains {len(analysis['images'])} images", style="bright_magenta")
    ...
    def show_images(self):
        ...
        for img in sorted(images):
            ...
            size_str = f"{size_mb:.2f} MB" if size_mb > 1 else f"{img.stat().st_size:,} bytes"
            modified_str = datetime.fromtimestamp(img.stat().st_mtime).strftime('%Y-%m-%d %H:%M')
    ...
    def manage_sticky_notes(self, action: str, notebook: Optional[str] = None, **kwargs):
                ...
                if notes:
                    ...
                    for note in notes:
                        notes_table.add_row(
                            note['id'],
                            note['category'].upper(),
                            note['content'][:60] + ("..." if len(note['content']) > 60 else ""),
                            note['created'][:10]
                        )
                    ...
                    console.print(f"No sticky notes found for [bold]{notebook}[/bold]", style="yellow")
            else:
                # List all notes
                ...
                for nb, notes in self.notes_manager.notes.items():
                    if notes:
                        ...
                        for note in notes[:3]:  # Show first 3
                            content = note['content'][:40] + ("..." if len(note['content']) > 40 else "")
    ...
    def list_notebooks(self):
        ...
        if not self.analyzer.notebooks:
            console.print("No Jupyter notebooks found in the current directory.", style="red")
            return
        ...
        for notebook in track(sorted(self.analyzer.notebooks), description="Analyzing notebooks..."):
            analysis = self.analyzer.analyze_notebook(notebook)
    ...
    def analyze_specific_notebook(self):
        ...
        console.print(Panel(choices_table, title="Available Notebooks", border_style="blue"))
    ...
    def interactive_notes(self):
        ...
        console.print(Panel("📝 Sticky Notes Manager", style="bold cyan", box=box.DOUBLE))
        ...
        if choice == "1":
            ...
            console.print(Panel(choices_table, title="Choose Notebook", border_style="blue"))
            ...
            self.manage_sticky_notes("add", notebook, content=content, category=category)
        elif choice == "3":
...
Examples:
  ...
  %(prog)s --notes add notebook.ipynb "Important finding"
    ...
    parser.add_argument('--list', action='store_true', help='List all notebooks with summaries')
    ...
    parser.add_argument('--notes', nargs='+', help='Sticky notes: list | add <notebook> <content>')
    ...
    if args.list:
        ...
        if args.notes[0] == "list":
            ...
            console.print("Usage: --notes list [notebook] | --notes add <notebook> <content>", style="red")
    elif args.notebook:
        # Analyze specific notebook
