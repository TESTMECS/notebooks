...
def minkowski_ds2_with_phi(t1, x1, t2, x2, phi_model):
    ...
    coords = torch.cat([t1.unsqueeze(-1), x1], dim=-1)  # assume x1 is (N, 3), so shape becomes (N, 4)
...
def project_until_convergence(pairs, spatial, time_vec, phi_model, eps1=1e-5, max_passes=10000):
    """
...
def train_phi_from_ds2_violations(phi_model, pairs, spatial, time_vec, eps1=1e-5, num_epochs=200, lr=1e-4):
    """
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch+1}/{num_epochs}, Avg phi loss: {total_loss / len(pairs):.6f}")
...
edge_index_gnn = torch.tensor([(p, c) for c, p in hierarchy_pairs], dtype=torch.long, device=device).t().contiguous()
if edge_index_gnn.nelement() == 0: # Handle case with no pairs for GNN by creating a self-loop for node 0
    edge_index_gnn = torch.tensor([[0],[0]], dtype=torch.long, device=device) if N > 0 else torch.empty((2,0), dtype=torch.long, device=device)
...
edge_index_causal_loss = torch.tensor(hierarchy_pairs, dtype=torch.long, device=device).t().contiguous()
...
phi_model = train_phi_from_ds2_violations(
    phi_model, hierarchy_pairs, spatial, time_vec, eps1=1e-5, num_epochs=50, lr=1e-3 # Reduced epochs for demo
)
...
node_features = torch.cat([adjusted_time_vec.unsqueeze(1), spatial, phi_values], dim=1) # (N, 1+D_spatial+1)
...
for epoch in range(num_gnn_epochs):
    ...
    if epoch % 10 == 0 or epoch == num_gnn_epochs - 1:
        print(f"GNN Epoch {epoch+1}/{num_gnn_epochs}, Task Loss: {task_loss.item():.4f}, Causal Phi Loss: {c_phi_loss.item():.4f}, Total Loss: {total_loss.item():.4f}")
...
if not all_synsets:
    raise ValueError("No synsets found. Check WordNet installation or subset selection.")
...
gnn_targets = torch.tensor(original_depths_list, dtype=torch.float32, device=device).unsqueeze(1)
...
if not edge_index_gnn_list and N > 0: # Handle case with no pairs for GNN
    ...
    edge_index_gnn = torch.tensor(edge_index_gnn_list, dtype=torch.long, device=device).t().contiguous()
...
print(f"Initial min time_vec value: {time_vec.min().item()}, max: {time_vec.max().item()}")
...
if edge_index_causal_loss.shape[1] > 0 : # Only train if there are pairs
    phi_model = train_phi_from_ds2_violations(
        phi_model, hierarchy_pairs, spatial, time_vec,
        eps1=phi_eps1, num_epochs=phi_epochs, lr=phi_lr
    )
    print("phi_model training complete.\n")
else:
...
if edge_index_causal_loss.shape[1] > 0:
    ...
    print("Skipping time projection as there are no hierarchy pairs. Using original time_vec.")
    adjusted_time_vec = time_vec.clone()

print(f"Original time_vec stats: min={time_vec.min().item():.2f}, max={time_vec.max().item():.2f}, mean={time_vec.mean().item():.2f}")
print(f"Adjusted time_vec stats: min={adjusted_time_vec.min().item():.2f}, max={adjusted_time_vec.max().item():.2f}, mean={adjusted_time_vec.mean().item():.2f}\n")
# %%
...
for epoch in range(num_gnn_epochs):
    ...
    if edge_index_causal_loss.shape[1] > 0 and lambda_phi_loss_weight > 0: # Only compute if pairs exist and weight > 0
        c_phi_loss = causal_phi_loss(
            node_reps=spatial, # Original spatial coordinates
            time_vec=adjusted_time_vec,
...
with torch.no_grad():
    ...
    for i in range(min(5, N)):
        print(f"Synset: {id_to_synset[i].name()}, Original Depth: {gnn_targets[i].item():.2f}, Predicted Depth: {final_predictions[i].item():.2f}, Adjusted Time: {adjusted_time_vec[i].item():.2f}, Phi: {phi_values[i].item():.2f}")
# %%
# === NEW: high-level causal refinement pipeline ===
def causal_refine_embeddings_with_phi(BERT_embeddings, attention_matrix, phi_model, epsilon=1e-5):
    """
    Args:
    ...
    time_vec = project_until_convergence(pairs, spatial, time_vec, phi_model, eps1=epsilon)
    ...
    refined_embeddings = torch.cat([spacetime_coords, BERT_embeddings[:, 3:]], dim=-1)  # prepend causal txy
...
def project_until_convergence(pairs, spatial, time_vec, phi_model, eps1=1e-5, max_passes=10000):
    ...
    time_vec = time_vec.clone().detach().requires_grad_(False) # Ensure it's on the correct device
    spatial = spatial.clone().detach().requires_grad_(False) # Ensure it's on the correct device
    ...
    while not converged and num_passes < max_passes:
        ...
        for (i, j) in pairs: # i is child, j is parent
            ...
            if delta_t <= 0: # Child must be after parent
                ...
                time_vec[i] = t_j + torch.sqrt( (dx2 + eps1) / (phi_i + 1e-9) + 1e-9) # Ensure some minimal separation
            ...
            if ds2 >= -eps1: # Causal violation or too close to light cone
                ...
                sqrt_arg = (dx2 + eps1) / (phi_i + 1e-9) # Add small epsilon to phi_i to prevent division by zero
                if sqrt_arg < 0: # Should not happen if phi_i > 0 and dx2+eps1 > 0
                    sqrt_arg = torch.tensor(1e-9, device=time_vec.device) # Minimal positive value

                new_delta_t = torch.sqrt(sqrt_arg + 1e-9) # Add small epsilon for sqrt stability
                time_vec[i] = t_j + new_delta_t.item()
    ...
    if num_passes == max_passes and not converged:
        print(f"Warning: project_until_convergence reached max_passes ({max_passes}) without full convergence.")
    # else:
...
# === NEW: high-level causal refinement pipeline ===
def causal_refine_embeddings_with_phi(BERT_embeddings, attention_matrix, phi_model,
                                      initial_time_strategy="zero", k_attention=2,
                                      phi_input_dims=3, # Number of spatial dimensions phi_model expects
                                      epsilon=1e-5, max_passes_projection=1000):
    """
    ...
    if actual_spatial_dims < phi_input_dims:
        print(f"Warning: BERT_embeddings hidden_dim ({hidden_dim}) is less than "
    ...
    if actual_spatial_dims == 0:
        raise ValueError("Cannot extract spatial dimensions if hidden_dim is 0 or phi_input_dims is 0.")

    spatial = BERT_embeddings[:, :actual_spatial_dims].clone().detach() # (seq_len, actual_spatial_dims)
    ...
    if initial_time_strategy == "zero":
        ...
        for i in range(1, seq_len):
            time_vec[i] = torch.sum(attention_matrix[i, :i]) # Sum attention from i to j where j < i
    ...
    for i in range(seq_len): # i is the potential child
        ...
        for k_idx in range(current_k):
            j = topk_indices_relative_to_past[k_idx].item() # j is an index in the range [0, i-1]
    ...
    if pairs: # Only run projection if there are pairs to enforce
        time_vec = project_until_convergence(pairs, spatial, time_vec, phi_model,
                                             eps1=epsilon, max_passes=max_passes_projection)
    else:
    ...
    refined_spacetime_coords = torch.cat([time_vec.unsqueeze(-1), spatial], dim=-1) # (seq_len, 1 + actual_spatial_dims)
    ...
    refined_embeddings = torch.cat([refined_spacetime_coords, remaining_bert_dims], dim=-1)
...
PHI_MODEL_SPATIAL_DIMS = 3 # How many spatial dimensions phi_model expects (e.g., x, y, z)
...
for i in range(SEQ_LEN):
    for j in range(i): # Token i attends to token j (j < i)
        dummy_attention_matrix[i, j] = torch.rand(1).item() * (1.0 / (i - j)) # Higher for closer tokens
...
for i in range(SEQ_LEN):
    ...
    if row_sum_past > 0 and i > 0 :
         ...
         if i > 0 : dummy_attention_matrix[i, :i] = 1.0/i


print(f"Dummy BERT embeddings shape: {dummy_bert_embeddings.shape}")
print(f"Dummy attention matrix (sum of first row's past attention): {dummy_attention_matrix[1,0] if SEQ_LEN >1 else 'N/A'}")
...
if refined_embs.shape[1] == 1 + dummy_bert_embeddings.shape[1]:
    ...
    spatial_match = torch.allclose(refined_embs[0,1:1+bert_tail_start_dim], refined_coords[0,1:])
    tail_match = torch.allclose(refined_embs[0, 1+bert_tail_start_dim:], dummy_bert_embeddings[0, bert_tail_start_dim:])
    ...
    if time_match and spatial_match and tail_match:
        ...
        print(f"Time match: {time_match}, Spatial match: {spatial_match}, Tail match: {tail_match}")

else:
    print(f"Unexpected shape for refined_embeddings. Expected {1 + dummy_bert_embeddings.shape[1]}, got {refined_embs.shape[1]}")
# %% [markdown]
"""
Visualizations
"""
...
original_spatial_np = dummy_bert_embeddings[:, :PHI_MODEL_SPATIAL_DIMS].cpu().detach().numpy()
...
with torch.no_grad():
    phi_values_at_refined = demo_phi_model(refined_coords.to(device)).cpu().detach().numpy()

# --- Plot 1: Refined Spacetime Coordinates (2D: t vs x) ---
plt.figure(figsize=(10, 6))
scatter = plt.scatter(x_refined, t_refined, c=phi_values_at_refined, cmap='viridis', s=50)
for i in range(SEQ_LEN):
    plt.text(x_refined[i] + 0.05, t_refined[i] + 0.05, str(i), fontsize=9) # Label points with token index
plt.xlabel(f"Spatial Dim 1 (from BERT emb dim 0)")
...
if PHI_MODEL_SPATIAL_DIMS >= 2:
    ...
    scatter3d = ax.scatter(x_refined, y_refined, t_refined, c=phi_values_at_refined, cmap='viridis', s=50)
    for i in range(SEQ_LEN):
        ax.text(x_refined[i], y_refined[i], t_refined[i], str(i), fontsize=9)
    ax.set_xlabel(f"Spatial Dim 1 (BERT emb dim 0)")
    ax.set_ylabel(f"Spatial Dim 2 (BERT emb dim 1)")
...
if initial_time_strategy_used == "zero":
    ...
    print("Note: 'attention_based' initial time for plotting requires dummy_attention_matrix. Using sequential for simplicity here.")
...
plt.plot(token_indices, initial_time_vec_np, 'o-', label='Initial Time (e.g., Sequential)', color='blue')
plt.plot(token_indices, t_refined, 's-', label='Refined Time (after projection)', color='red')
for i in range(SEQ_LEN):
    plt.plot([token_indices[i], token_indices[i]], [initial_time_vec_np[i], t_refined[i]], linestyle='--', color='gray', alpha=0.7)
...
plt.figure(figsize=(10, 6))
plt.plot(x_refined, t_refined, 'o-', label='Token Trajectory', color='purple', markersize=8, markerfacecolor='orange')
for i in range(SEQ_LEN):
...
if PHI_MODEL_SPATIAL_DIMS >= 2:
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(x_refined, y_refined, t_refined, 'o-', label='Token Trajectory', color='purple', markersize=8, markerfacecolor='orange')
    for i in range(SEQ_LEN):
        ax.text(x_refined[i], y_refined[i], t_refined[i], str(i), fontsize=9)
...
def project_until_convergence(pairs, spatial, time_vec, phi_model, eps1=1e-5, max_passes=100):
        ...
        for i_child, i_parent in pairs:
            if i_child < len(time_vec_clone) and i_parent < len(time_vec_clone):
                 if time_vec_clone[i_child] <= time_vec_clone[i_parent]:
                    time_vec_clone[i_child] = time_vec_clone[i_parent] + 0.1 * torch.rand(1).to(time_vec.device)
...
class MotionFeatureExtractor(nn.Module):
    def __init__(self, input_channels=1, output_motion_dim=64, num_frames_in_clip=3):
        ...
        self.spatial_conv = nn.Conv2d(input_channels * num_frames_in_clip, 32, kernel_size=3, padding=1)
        self.temporal_pool = nn.AdaptiveAvgPool1d(1) # To simulate temporal aggregation over the clip
        self.fc = nn.Linear(32, output_motion_dim) # Assuming pooling flattens spatial dims
    ...
    def forward(self, frames_sequence):
        ...
        T_motion_steps = T_total - self.num_frames_in_clip + 1 if T_total >= self.num_frames_in_clip else 1
        ...
        for t in range(T_motion_steps):
            clip = frames_sequence[:, t : t + self.num_frames_in_clip] # (B, num_frames_in_clip, C, H, W)
            clip = clip.reshape(B, self.num_frames_in_clip * C, H, W) # Treat frames in clip as channels
        ...
        motion_features = torch.stack(motion_features_list, dim=1) # (B, T_motion_steps, D_motion)
...
class TemporalAttentionModule(nn.Module):
    def __init__(self, motion_dim, num_heads=4, ff_dim=128):
        super().__init__()
        self.attention = nn.MultiheadAttention(embed_dim=motion_dim, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, motion_features):
        ...
        attn_output, attn_weights = self.attention(motion_features, motion_features, motion_features)
...
class CaMPNet(nn.Module):
    def __init__(self, input_channels=1, motion_dim=64, num_frames_in_clip=3,
                 phi_spatial_dim=3, num_attn_heads=4, ff_dim=128,
                 num_classes=10, k_attention=2):
        super().__init__()
        self.motion_extractor = MotionFeatureExtractor(input_channels, motion_dim, num_frames_in_clip)
        self.temporal_attention = TemporalAttentionModule(motion_dim, num_attn_heads, ff_dim)
    ...
    def forward(self, frames_sequence, targets=None, lambda_phi_loss_weight=0.1):
        ...
        # Phase 1
        motion_features = self.motion_extractor(frames_sequence) # (B, T_motion, D_motion)
        # print(f"Debug: motion_features shape: {motion_features.shape}")

        # Phase 2
        context_motion_features, attention_weights = self.temporal_attention(motion_features)
        ...
        for b_idx in range(B):
            current_motion_features = context_motion_features[b_idx] # (T_motion, D_motion)
            ...
            spatial_coords_for_phi = self.spatial_projector(current_motion_features.unsqueeze(0)).squeeze(0) # (T_motion, phi_spatial_dim)
            # print(f"Debug: spatial_coords_for_phi shape: {spatial_coords_for_phi.shape}")


            # Initial time vector
            initial_time_vec = torch.arange(T_motion, dtype=torch.float32, device=frames_sequence.device)
            ...
            for t_step in range(T_motion):
                ...
                phi_input = torch.cat([t_refined, s_t.unsqueeze(0)], dim=-1) # (1, 1 + phi_spatial_dim)
                phi_val = self.phi_model(phi_input).unsqueeze(0) # (1,1) because phi_model squeezes last dim
            ...
            if pairs:
                ...
                edge_index_causal = torch.tensor(pairs, dtype=torch.long, device=frames_sequence.device).t()

                phi_loss_item = causal_phi_loss(
                    spatial_coords_for_phi, adjusted_time_vec, edge_index_causal, self.phi_model
                )
        ...
        final_augmented_features = torch.stack(batch_augmented_features) # (B, T_motion, D_augmented)
...
dummy_video_data = torch.randn(BATCH_SIZE, TOTAL_FRAMES, FRAME_H, FRAME_W, FRAME_C, device=device)
...
print(f"CaMPNet instantiated. Number of parameters: {sum(p.numel() for p in camp_net.parameters() if p.requires_grad)}")
...
print("\n--- Forward Pass ---")
logits, total_loss, task_loss, phi_loss_contrib = camp_net(dummy_video_data, dummy_targets)
...
if total_loss is not None:
    ...
    if total_loss.requires_grad:
        ...
        if not task_loss.requires_grad: print("Task loss also no grad.")
        if phi_loss_contrib and not phi_loss_contrib.requires_grad: print("Phi loss also no grad (if exists).")
...
def causal_phi_loss(node_reps, time_vec, edge_index, phi_model, eps1=1e-5):
    ...
    if num_edges > 0:
        ...
        return total_loss_mock / num_edges if num_edges > 0 else torch.tensor(0.0, device=node_reps.device, requires_grad=True)
    else:
        return torch.tensor(0.0, device=node_reps.device, requires_grad=True)
...
class MotionFeatureExtractor(nn.Module):
    ...
    def forward(self, frames_sequence):
        ...
        if T_motion_steps <= 0: # Handle very short sequences where no clips can be formed
             dummy_output_dim = self.fc.out_features
...
class SpatialProjector(nn.Module):
    def __init__(self, motion_dim, phi_spatial_dim=3):
        ...
        if motion_dim < phi_spatial_dim:
             print(f"Warning: motion_dim ({motion_dim}) is less than phi_spatial_dim ({phi_spatial_dim}). Projection will truncate or pad.")
...
class Decoder(nn.Module):
    ...
    def forward(self, augmented_features_sequence):
        ...
        if augmented_features_sequence.shape[1] == 0:
             ...
             dummy_logits = torch.zeros(augmented_features_sequence.shape[0], self.fc.out_features, device=augmented_features_sequence.device)
             return dummy_logits
...
class CaMPNet(nn.Module):
    ...
    def forward(self, frames_sequence, targets=None, lambda_phi_loss_weight=0.1):
        ...
        if T_motion == 0:
             ...
             dummy_logits = torch.zeros(B, self.decoder.fc.out_features, device=frames_sequence.device)
             return dummy_logits, torch.tensor(0.0, device=frames_sequence.device), 0.0, 0.0
        ...
        for b_idx in range(B):
            ...
            for t_step in range(T_motion):
                ...
                # Ensure inputs to phi_model are correctly shaped and on device
                phi_input = torch.cat([t_refined, s_t_unsqueeze], dim=-1) # Shape is (1, 1 + phi_spatial_dim)
                phi_val = self.phi_model(phi_input).unsqueeze(0) # phi_model returns (1,) after squeeze, unsqueeze back to (1,1)
            ...
            if not augmented_features_this_item: # Should not happen if T_motion > 0
                 ...
                 aug_dim = 1 + self.phi_spatial_dim + 1 + motion_features.shape[-1]
                 dummy_aug_feat = torch.zeros(aug_dim, device=frames_sequence.device)
                 batch_augmented_features.append(dummy_aug_feat.unsqueeze(0)) # Add time step dim
            ...
            # Calculate phi_loss for this item
            phi_loss_item = torch.tensor(0.0, device=frames_sequence.device, requires_grad=True) # Initialize to 0
...
# %%
    ...
    MOTION_DIM = 32
    NUM_FRAMES_IN_CLIP = 3 # For motion extractor. T_motion = TOTAL_FRAMES - NUM_FRAMES_IN_CLIP + 1
    PHI_SPATIAL_DIM = 3
    NUM_CLASSES = 5 # e.g., 5 different words/shapes to recognize
    K_ATTENTION_PAIRS = 1
...
class TemporalAttentionModule(nn.Module):
    def __init__(self, motion_dim, num_heads=4, ff_dim=128):
        ...
        self.norm1 = nn.LayerNorm(motion_dim)
        self.norm2 = nn.LayerNorm(motion_dim)
        self.ff = nn.Sequential(nn.Linear(motion_dim, ff_dim), nn.ReLU(), nn.Linear(ff_dim, motion_dim))
    def forward(self, motion_features):
        attn_output, attn_weights = self.attention(motion_features, motion_features, motion_features)
    ...
    def forward(self, frames_sequence, targets=None, lambda_phi_loss_weight=0.1):
        ...
        for b_idx in range(B):
            ...
            if T_motion == 0: # Handle case where no motion features are produced
                ...
                aug_dim = 1 + self.phi_spatial_dim + 1 + motion_features.shape[-1]
                dummy_aug_feat_seq = torch.zeros(1, aug_dim, device=frames_sequence.device)
            ...
            for i in range(T_motion):
                ...
                for k_val_idx in range(current_k):
                    j = topk_indices_relative[k_val_idx].item()
                    pairs.append((i, j))
            spatial_coords_for_phi = self.spatial_projector(current_motion_features.unsqueeze(0)).squeeze(0)
            initial_time_vec = torch.arange(T_motion, dtype=torch.float32, device=frames_sequence.device)
            self.phi_model.to(spatial_coords_for_phi.device) # Ensure phi_model on correct device
            adjusted_time_vec = project_until_convergence(
                pairs, spatial_coords_for_phi, initial_time_vec, self.phi_model
            )
            augmented_features_this_item = []
            for t_step in range(T_motion):
                ...
                aug_feat = torch.cat([t_refined, s_t, phi_val.squeeze(0), original_motion_vec])
                augmented_features_this_item.append(aug_feat)
            batch_augmented_features.append(torch.stack(augmented_features_this_item))
            if pairs:
                edge_index_causal = torch.tensor(pairs, dtype=torch.long, device=frames_sequence.device).t()
        ...
        if max_len > 0 : # Proceed only if there's something to pad
            for feat_seq in batch_augmented_features:
                if feat_seq.numel() == 0: # Handle completely empty feature sequences if they occur
                    pad_len = max_len
                else:
                    pad_len = max_len - feat_seq.shape[0]

                if pad_len > 0:
                    padding = torch.zeros(pad_len, feat_seq.shape[1], device=feat_seq.device)
                    padded_augmented_features.append(torch.cat([feat_seq, padding], dim=0))
        ...
        if not padded_augmented_features: # If everything was empty or became empty
             ...
             aug_dim_fallback = 1 + self.phi_spatial_dim + 1 + self.motion_extractor.fc.out_features # Example dim
             final_augmented_features = torch.zeros(B, 1, aug_dim_fallback, device=frames_sequence.device)
...
CATEGORY_FIELD_NAME = "label_text_content" # e.g. "label_text_content" for text category

class SpookyDataset(Dataset):
    def __init__(self, hf_dataset, target_frames, frame_h, frame_w, frame_c, label_map, category_field_name):
    ...
    def __getitem__(self, idx):
        ...
        video_path = item['video'] # This is usually how HF datasets provides video files
        ...
        num_actual_frames = random.randint(self.target_frames // 2, self.target_frames * 2) # Simulate variable length
        frames = [np.random.rand(64, 64, 3).astype(np.uint8) * 255 for _ in range(num_actual_frames)] # HWC, RGB dummy
        ...
        if current_num_frames > self.target_frames:
            ...
            padding = torch.zeros(self.target_frames - current_num_frames, FRAME_C, FRAME_H, FRAME_W)
        ...
        if label_id == -1:
            print(f"Warning: Unknown label '{label_str}' for item {idx}. Assigning to first class.")
            label_id = 0
...
if __name__ == '__main__':
    ...
    try:
        ...
        spooky_hf_dataset_train = load_dataset("timeblindness/spooky-bench", name="text", split='train[:10]') # 10 samples
        spooky_hf_dataset_test = load_dataset("timeblindness/spooky-bench", name="text", split='test[:10]')
        print(f"Loaded {len(spooky_hf_dataset_train)} train samples and {len(spooky_hf_dataset_test)} test samples for 'text' category.")
        ...
        print("Please ensure you have 'ffmpeg' and 'decord' installed and are connected to the internet.")
        print("You might also need to log in to Hugging Face if the dataset requires authentication: `huggingface-cli login`")
        exit()
    ...
    if CATEGORY_FIELD_NAME not in spooky_hf_dataset_train.features:
        print(f"Error: The expected label field '{CATEGORY_FIELD_NAME}' is not in the dataset features.")
...
print(f"Refined embeddings shape after causal_refine_embeddings_with_phi: {refined_embs.shape}")
...
print(f"New predictor instantiated with input_feature_dim={input_feature_dim_predictor}")
...
synthetic_targets = torch.sin(refined_coords[:, 0] * 2 * np.pi) + \
                    torch.cos(refined_coords[:, 1] * np.pi) + \
                    torch.sigmoid(refined_embs[:, 5].float()) * 2.0 # Use a feature from original BERT + spacetime
...
for epoch in range(num_predictor_epochs):
    ...
    if epoch % 20 == 0 or epoch == num_predictor_epochs - 1:
        print(f"Predictor Epoch {epoch+1}/{num_predictor_epochs}, MSE Loss: {loss.item():.6f}")
...
with torch.no_grad():
    ...
    for i in range(min(5, SEQ_LEN)):
        print(f"Token {i}: Target={synthetic_targets[i].item():.4f}, Prediction={final_predictions[i].item():.4f}")
        # Optionally, include refined time and spatial coords for context
        print(f"  (Refined t: {refined_coords[i,0].item():.4f}, x: {refined_coords[i,1].item():.4f}, y: {refined_coords[i,2].item():.4f})")
...
class CausalOptimizationProblem:
    def __init__(self, embedder, phi_model, hierarchy_pairs, lambda_phi=0.1):
        ...
        self.edge_index_causal = torch.tensor(hierarchy_pairs, dtype=torch.long, device=device).t().contiguous()
        if self.edge_index_causal.nelement() == 0:
             self.edge_index_causal = torch.empty((2,0), dtype=torch.long, device=device)
    ...
    def calculate_loss(self, node_features, gnn_edge_index, gnn_targets):
        ...
        if self.edge_index_causal.shape[1] > 0 and self.lambda_phi > 0:
             ...
             assert hasattr(self, 'phi_spatial_dim'), "CausalOptimizationProblem needs phi_spatial_dim"
             ...
             extracted_spatial = node_features[:, 1 : 1 + self.phi_spatial_dim] # (N, phi_spatial_dim)

             # Check dimensions before calling causal_phi_loss
             if extracted_spatial.shape[1] != self.phi_spatial_dim:
                  print(f"Error: Extracted spatial dims ({extracted_spatial.shape[1]}) != expected phi_spatial_dim ({self.phi_spatial_dim})")
...
if node_features.shape[1] < 1 + causal_opt_problem.phi_spatial_dim:
     raise ValueError(f"node_features shape ({node_features.shape}) is too small to extract time (1) and spatial ({causal_opt_problem.phi_spatial_dim}).")
...
for epoch in range(num_gnn_epochs):
    ...
    if epoch % 20 == 0 or epoch == num_gnn_epochs - 1:
        print(f"Opt Epoch {epoch+1}/{num_gnn_epochs}, Total Loss: {total_loss.item():.4f}, Task Loss: {task_loss_val:.4f}, Causal Phi Loss: {phi_loss_val:.4f}")
...
with torch.no_grad():
    ...
    # Compare some predictions to targets
    for i in range(min(5, N)):
        print(f"Synset: {id_to_synset[i].name()}, Original Depth: {gnn_targets[i].item():.2f}, Predicted Depth: {final_predictions[i].item():.2f}")
