...
def generate_brownian_bridge(
    ...
    if num_steps > 0:
        dW_scaled = sigma_scale * np.sqrt(dt) * np.random.normal(0, 1, num_steps)
    else:
        dW_scaled = np.array([])
    ...
    if T_prime == 0:
        if x_start == x_final:
            ...
            return tau_coords, np.linspace(x_start, x_final, num_steps + 1)

    W_scaled_T_prime = W_scaled[-1]
    brownian_bridge_component = W_scaled - (t_prime_coords / T_prime) * W_scaled_T_prime
    classical_path_segment = x_start + (x_final - x_start) * (t_prime_coords / T_prime)
...
for ax_idx, (current_scale, title_text) in enumerate(scales_and_titles):
    ax = axes[ax_idx]
    ...
    ax.grid(True)
    xlim_padding = (
        0.05 * (tau_final - tau_initial) if (tau_final - tau_initial) > 0 else 0.05
    )
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])  # Adjust layout for suptitle and figtext
fig.text(
    0.5,
    0.005,
...
def generate_minkowski_fluctuating_path(
    ...
    if num_steps == 0:
        return t_coords, np.array([x_start, x_end] if t_start != t_end else [x_start])
    ...
    dt_segment = (t_end - t_start) / num_steps if num_steps > 0 else 1.0
    ...
    if num_steps > 0:
        ...
        spatial_bridge_component = (
            spatial_deviations - time_fractions * spatial_deviations[-1]
        )
    else:
        spatial_bridge_component = (
            np.array([0.0]) if num_steps == 0 else np.array([0.0, 0.0])
        )
...
print(f"Spacetime interval for classical path: dt={dt:.2f}, dx={dx:.2f}")
print(f"dt^2 = {dt**2:.3f}, dx^2 = {dx**2:.3f}")
if dt**2 < dx**2:
    print("Warning: Classical path is spacelike! Child not in causal future of parent.")
    print(
        "This setup might not perfectly reflect the paper's causal embedding intention for the classical path."
    )
    print("For the visualization, we will proceed, but keep this in mind.")
elif dt**2 == dx**2:
...
m_c = 0.1  # Mass of child concept (lighter, so it orbits more visibly)
epsilon_soft = 0.01  # Softening factor for gravity to avoid singularity

# Initial Conditions for the classical system
t_start = 0.5
t_end = 1.5  # Increased t_end to see more of the "orbit"
num_t_steps = 200  # Number of steps in semantic time for classical path
...
num_sample_paths = 20
perturbation_strength_parent = 0.02  # Fluctuation around parent's mean path
perturbation_strength_child = 0.03  # Fluctuation around child's gravitational path
...
def generate_brownian_bridge_component(num_steps, scale):
    ...
    dt_norm = 1.0 / num_steps  # Normalized time step for bridge construction

    # Increments for a scaled Wiener process
    dW_scaled = scale * np.sqrt(dt_norm) * np.random.normal(0, 1, num_steps)
...
child_x_classical[0] = parent_x_classical_mean[0] + child_x_initial_offset
...
for i in range(num_t_steps):
    ...
    if abs(r_x) < 1e-9:  # Avoid division by zero if perfectly aligned
        ...
        force_g_x = (
            -G_s * m_p * m_c * r_x / (dist_sq_soft * np.sqrt(dist_sq_soft))
        )  # F = -GMm r_vec / |r|^3
    ...
    child_vx_total_current = child_vx_total_current + accel_c_x * dt_sim
    child_x_classical[i + 1] = child_x_classical[i] + child_vx_total_current * dt_sim

print("Classical path calculation complete.")
print(
...
for i in range(num_sample_paths):
    ...
    parent_x_fluctuating = parent_x_classical_mean + parent_fluctuations
...
child_vx_initial_relative = -0.1  # Initial relative velocity of child to parent
...
# Rule 2: Repulsion
G_s_repulsion = 0.005  # Repulsion constant (can be different from gravity)
    ...
    for i in range(num_t_steps):
        ...
        if rule_type == "gravity":
            ...
            if abs(dist_cubed_soft) > 1e-9:
                force_g_x = -G_s_gravity * m_p * m_c * r_x / dist_cubed_soft
                accel_c_x = force_g_x / m_c
            ...
            target_x_offset = A_wave * np.sin(omega_wave * (t_i - t_start) + phi_wave)
            ...
            vx_target_offset_derivative = (
                A_wave * omega_wave * np.cos(omega_wave * (t_i - t_start) + phi_wave)
            )
            vx_child_target_wave = parent_vx_mean_val + vx_target_offset_derivative

            force_channel_x = -k_channel * (xc_i - x_child_target) - damping_channel * (
        ...
        child_x_classical[i + 1] = child_x_classical[i] + child_vx_abs_current * dt_sim
...
plt.tight_layout(rect=[0, 0.06, 1, 0.95])  # Adjust for suptitle and legend
plt.show()
...
# Common
t_start = 0.5
t_end = 1.5  # Adjusted from 2.0 to focus on the first "swoop" for tanh
...
child_x_initial_offset = 0.1
child_vx_initial_relative = -0.2  # This value is key for the "swoop" shape
...
def simulate_child_classical_path_gravity(
    ...
    child_vx_abs_current = parent_vx_mean_val + child_vx_initial_relative
    child_x_classical_sim[0] = parent_x_mean_path[0] + child_x_initial_offset
...
def model_sigmoid_derivative_like(t, Amp, k, t0, offset):
    ...
    return model_bell_curve(t, Amp, k, t0, offset)  # k here is mu, t0 is sigma for bell
...
try:
    ...
    y_fit_bell = np.full_like(child_x_gravitational_path, np.nan)  # Placeholder
...
if fit_success_bell:
    ...
    print("  If Amp is negative, it's fitting a 'dip' or 'valley' in the path.")
    print(
...
def predict_is_child_by_tanh_swoop(
    ...
    t_coords_classical_demo = np.linspace(0.5, 1.5, 201)  # Match previous example
    ...
    child_vx_abs_current = (
        parent_x_classical_mean_demo[1] - parent_x_classical_mean_demo[0]
    ) / (
        t_coords_classical_demo[1] - t_coords_classical_demo[0]
    ) + child_vx_initial_relative_demo
    child_x_simulated[0] = parent_x_classical_mean_demo[0] + child_x_initial_offset_demo
    dt_sim_demo = t_coords_classical_demo[1] - t_coords_classical_demo[0]
    ...
    for i in range(len(t_coords_classical_demo) - 1):
        ...
        accel_c_x = 0.0
        if abs(dist_cubed_soft) > 1e-9:
            force_g_x = -G_s_gravity_demo * m_p_demo * m_c_demo * r_x / dist_cubed_soft
    ...
    try:
        ...
        default_initial_guess_tanh = [
            (np.min(child_x_simulated) - np.max(child_x_simulated)) / 2,  # A
            5,  # B
        ...
        print("Tanh fit failed for this pair.")
    ...
    elif fit_successful and A_fitted > abs(A_threshold):  # Example for repulsion
...
prediction_rock = (
    "Uncertain / No clear child-like attractive swoop"  # if A_fitted was positive
)
...
DT_SIM = (SIMULATION_T_END - SIMULATION_T_START) / SIMULATION_NUM_T_STEPS

# Semantic Physics Parameters (THESE ARE HYPOTHETICAL AND NEED TUNING/DERIVATION)
G_S_GRAVITY = 0.05  # Universal semantic gravitational constant (example)
...
EMBEDDING_DIM = 50  # Example dimension if using simple random embeddings
...
def get_synset_embedding(synset):
    ...
    if USE_GLOVE:
        ...
        for lemma in synset.lemmas():
            try:
                vecs.append(
                    glove_vectors[lemma.name().lower().replace("_", " ")]
        ...
        if vecs:
            ...
            return np.random.rand(EMBEDDING_DIM) * 0.1  # Small random vector
...
def get_wordnet_pairs(num_positive=1000, num_negative_ratio=1):
    ...
    all_synsets = list(wn.all_synsets("n"))  # Focus on nouns for simplicity
    if not all_synsets:
        print(
        ...
        if hyponyms:
            for s_child in hyponyms:
                ...
                if len(positive_pairs) >= num_positive:
                    break
            processed_parents.add(s_parent)  # Avoid over-representing prolific parents
        if len(positive_pairs) >= num_positive:
            break
...
def derive_initial_conditions(parent_synset, child_synset, parent_emb, child_emb):
    """
    ...
    m_c = DEFAULT_M_C * (1 + random.uniform(-0.05, 0.05))  # Add small noise
    ...
    initial_vx_rel = -0.15 - 0.1 * initial_offset + random.uniform(-0.05, 0.05)
...
def simulate_child_trajectory_for_pair(parent_synset, child_synset):
    ...
    m_p, m_c, initial_offset, initial_vx_rel = derive_initial_conditions(
    ...
    child_x_classical = np.zeros_like(t_coords)
    child_vx_abs_current = parent_vx_mean_val + initial_vx_rel  # Absolute velocity
...
def extract_features_from_trajectory(t_coords, child_x_trajectory):
    ...
    initial_guess = [
        initial_A_guess,  # A
        5 / (t_coords[-1] - t_coords[0]),  # B (scales with time range)
        np.mean(t_coords),  # t0
        mean_x,  # C
    ]
    ...
    try:
        ...
        return [0.0]  # Default feature if fit fails (e.g., A=0 means no clear swoop)
...
if __name__ == "__main__":
    ...
    for i, (p_syn, c_syn) in enumerate(positive_pairs):
        if (i + 1) % 50 == 0:
            print(f"  Processed {i + 1}/{len(positive_pairs)} positive pairs...")
        t_coords, child_x = simulate_child_trajectory_for_pair(p_syn, c_syn)
    ...
    for i, (s1, s2) in enumerate(negative_pairs):
        if (i + 1) % 50 == 0:
            print(f"  Processed {i + 1}/{len(negative_pairs)} negative pairs...")
    ...
    if len(np.unique(y)) < 2:
        print("Only one class present in the dataset. Cannot train classifier.")
        exit()
    ...
    if X.shape[1] > 0:  # Check if features were extracted
        ...
        plt.title("Distribution of Feature 'A' by Class (Training Data)")
        plt.legend()
        plt.grid(True)
        plt.show()
    ...
    print("1. Derivation of initial conditions & masses from embeddings (crucial!).")
    print("2. Quality and source of synset embeddings.")
    print("3. More sophisticated negative sampling for WordNet pairs.")
    print(
        "4. More features from the trajectory (e.g., fit quality, other tanh params)."
    )
    print("5. Tuning of 'semantic physics' constants (Gs, default masses).")
...
USE_GLOVE = True  # Set to True to attempt using GloVe, False for random
# Download glove
# !wget http://nlp.stanford.edu/data/glove.6B.zip
# !unzip glove.6B.zip
GLOVE_FILE_PATH = "./glove.6B.50d.txt"  # <--- *** IMPORTANT: SET THIS PATH ***
EMBEDDING_DIM = (
    50  # Must match the dimension of your GloVe file (e.g., 50d, 100d, 200d, 300d)
)
...
if USE_GLOVE:
    try:
        ...
        if glove_vectors is None:
            print("Failed to load GloVe vectors. Falling back to random embeddings.")
...
def get_synset_embedding(synset):
    ...
    if USE_GLOVE and glove_vectors is not None:
        ...
        for lemma in synset.lemmas():
            try:
                vecs.append(glove_vectors[lemma.name().lower().replace("_", " ")])
            except KeyError:
                try:
                    vecs.append(glove_vectors[lemma.name().lower()])
                except KeyError:
...
def derive_initial_conditions(parent_synset, child_synset, parent_emb, child_emb):
    ...
    # Initial spatial offset: based on embedding distance?
    # For 1D simulation, we need to project. Let's use a component or distance.
    if parent_emb is not None and child_emb is not None and EMBEDDING_DIM > 0:
        ...
        initial_offset = distance / np.sqrt(EMBEDDING_DIM)  # Basic scaling
        initial_offset = np.clip(
...
def simulate_child_trajectory_for_pair(parent_synset, child_synset):
    ...
    if not isinstance(parent_emb, np.ndarray) or not isinstance(child_emb, np.ndarray):
        print(
        ...
        return t_coords, np.full_like(t_coords, np.nan)  # Return NaNs for trajectory
...
def extract_features_from_trajectory(t_coords, child_x_trajectory):
    ...
    if not np.all(np.isfinite(child_x_trajectory)):
        ...
        return [0.0]  # Return default feature if trajectory is invalid
...
if __name__ == "__main__":
    ...
    # Optional: Visualize distribution of feature 'A' for the two classes
    if X.shape[1] > 0:  # Check if features were extracted (should be 1 column)
        plt.figure(figsize=(10, 6))
    ...
    print("Key areas for improvement/exploration:")
    print("1. Derivation of initial conditions & masses from embeddings (crucial!).")
    print("2. Quality and source of synset embeddings (GloVe vs Random vs BERT etc.).")
    print("3. More sophisticated negative sampling for WordNet pairs.")
    print(
...
def derive_initial_conditions(parent_synset, child_synset, parent_emb, child_emb):
    ...
    if (
        ...
        initial_offset = np.clip(
            euclidean_dist / (np.sqrt(EMBEDDING_DIM) * 2 if USE_GLOVE else 5.0),
            0.05,
            0.3,
        )
...
def extract_features_from_trajectory(t_coords, child_x_trajectory):
    ...
    if (
        ...
        and abs(child_x_trajectory[0] - mean_x) > abs(child_x_trajectory[-1] - mean_x)
    ):
        initial_A_guess = delta_x / 2
    initial_guess = [
        initial_A_guess,
    ...
    try:
        ...
        return [0.0]
    except ValueError:  # Can happen if inputs to curve_fit are bad (e.g. all NaNs)
        return [0.0]
...
if dataset:
    ...
    if positive_pairs:
        ...
        try:
            sample_t_coords, sample_child_x_traj = simulate_child_trajectory_for_pair(
            ...
            print(f"An error occurred during sample simulation/feature extraction: {e}")
    else:
        print(
...
features["dot_product"] = np.sum(parent_embeddings * child_embeddings, axis=1)
...
for i in range(num_samples):
    ...
    x_traj = np.linspace(0, 1, num_points) + np.random.randn(num_points) * 0.1
...
for traj in dummy_trajectories:
    ...
    path_dist = np.sum(np.sqrt(np.diff(t_data) ** 2 + np.diff(x_data) ** 2))
    ...
    if duration > 1e-8:
        mean_speed = (
            np.sqrt((t_data[-1] - t_data[0]) ** 2 + (x_data[-1] - x_data[0]) ** 2)
            / duration
        )
...
def derive_initial_conditions_refined(parent_emb, child_emb, t_parent, t_child):
    """
    ...
    initial_offset_x = diff_norm  # Or a scaled version: diff_norm * some_scale
    ...
    if diff_norm > 1e-8:
        ...
        projection_magnitude = np.dot(child_emb, diff_vector_normalized)
        ...
        initial_vx_rel_x = (
            projection_magnitude  # Or projection_magnitude * some_velocity_scale
        )
...
print(f"t_parent: {t_parent_sample:.2f}, t_child: {t_child_sample:.2f}")
print(f"Derived Initial Spatial Offset (x_child - x_parent): {initial_offset:.4f}")
print(f"Derived Initial Relative Velocity (vx_child - vx_parent): {initial_vx_rel:.4f}")
# %% [markdown]
...
print("Target: House values in hundreds of thousands of dollars")
print("\nFirst few rows:")
...
try:
    ...
    print(f"Made {len(taxi_predictions)} predictions on taxi data sample")
...
print("To use your own dataset, modify the code in the OPTION 3 section above.")
